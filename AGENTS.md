...
## 18 septembre 2026 — clarification « cliffs » et nouveau lot Métano

L’utilisateur demandait « sprite de pond », puis a explicitement clarifié : **des cliffs comme les images référencées qui avaient servi aux falaises**. Ni ponts ni étangs. Lot non destructif `renders/cliffs_metano_v1/` : deux sprites (Cap des Alizés, Balcon du Levant), deux terrains de zone (Défilé des Explorateurs, Terrasses du Sillage). Références réellement envoyées : scène `source/falaises_generees/reference_canonique.png`, matière stricte V4, crochet droit V4 pour les sprites, témoin Métano pour les zones. Quatre générations puis deux corrections ; les six bruts restent conservés. La correction du cirque a ouvert le mur arrière : le nom Défilé reconnaît la composition réelle.

Méthode des rendus magenta Caps/Terrasses V4 : détourage, palette 328 RGB natifs par CIELAB, nuit Abyss, zéro resampling du terrain (1264×848), PNG jour/nuit et huit ORA à quatre plans (ciel, nuages, océan phase0, **terrain complet**). Herbe/roche/bordures ne sont pas séparées ; ne pas prétendre à des objets tous indépendants ou un sol caché reconstruit. Contexte océan V3 repris en nearest pour démonstration seulement, 64phases50ms. Pas de nouveaux pixels natifs reconstruits, pas d’extraction nouvelle spécifique Bourg-Trésor, pas de Ground/collisions/test moteur. ImportPNG8px possible dimensionnellement mais échelle visuelle à calibrer. Candidats inspectés, non encore approuvés ; cap plus grossier que la référence, terrasses avec sable/banquette ajoutés par le générateur.

Aperçu `apercu_cliffs_metano_v1.html`, planche et kitZIP22MiB dans le dossier de rendus ; sources `source/cliffs_metano_v1/`. 56contrôlesPASS (sources natives décodées, palette, alpha, nuit, ORA, contexte), logique de galerie via DOMsimulé et HTTP. Chromium non téléchargé (TLS/CDN), donc pas de test vrai navigateur. Aucun ancien rendu/mod modifié.


## Correction suivante — ciel/nuages validés + entrées de glace multi-layout

L’utilisateur rejette les ciels/nuages CapsV3 du premier lot de cliffs, demande **la famille de nuages et le ciel validés**, un push, puis une zone sud→grotte nord en plusieurs layouts de glace, avec sol/chemin, reliefs, immersion, ciel, nuageswrapoverlay, étoiles et frames d’onde boréale.

Sources correctes identifiées : **c16efe12, Guilde/Sharpedo**, `source/cote_dix_zones/reference_autre_agent/`. Helper `source/ciels_valides.py` reprend la moitié gauche du ciel sans lune cuite, les six rectangles natifs de nuages déplacés sur1440×208, nuit Guilde/Sharpedo et étoiles natives. Aucun resampling ciel/nuages/étoiles. Nuageswrap−4px/s, boucle360s. Ne plus reprendre les fonds CapsV3 pour ces lots. `cliffs_metano_v1` corrigé : PNGterrain jour/nuit inchangés, ORA5plans avec astres, scènes/ZIP/viewer actualisés ;60tests fichiersPASS et DOMsimuléPASS.

`renders/entrees_glace_v1/`, sourceshomonymes, aperçu `apercu_entrees_glace_v1.html` : **3layouts** (direct, courbe autour d’un massif, approcheenS) vers une grotte. Générations complètes référencées par `iceroadpmdsky.png` et `pmdskyicearena.png`, pas une mosaïque de captures. Septbruts sauvegardés (3maps,2correctionsduCol,2sols). Utiliser le Colfinal (chemin connecté au sud, sommet non coupé) et `sol_neige_doux`, pas le premier solgranuleux. Palette des deux références, dessins redessinés non certifiésnatifs. Normalisation uniforme1264×848→763×512, pose(2,128) dans768×640. Cinq plans de terrain enjour/nuit : solcomplet+chemin, profondeurdegrotte,cliffs/reliefs,immersiongauche/droite. Solcachégénéré, facescachéesnonreconstruites. Neufplans dans6ORA avec climat.

Ondeprincipale :10framesV10texturecanonique,160ms, petitescomposantes<40px retirées pour séparer pointsétoilés, pasdewrap. Alternative8framespalettecyclingV12copiéesbyte-identiques,120ms,dessingénéré ; aucun cielV3repris. Astres/nuages/cielindépendants. GIF/WebPdescène : ondesseules,nuagesphase0fixes ; vraiwrapindépendant dansviewer/recette.140contrôlesfichiersPASS donttroiscorridors13px jusqu’auseuil ;19testsgalerieDOMsimuléPASS. Aucun navigateur réel, runtimeGPU,collision,warpouintégrationPMDOvalidé. Candidatsvisuelsnonencoreapprouvés. Autreszones/programmeguildenonterminés.


## Suite — onde boréale seule générée, couleur et ondulation V2

Nouvelle livraison `renders/onde_boreale_glace_v2/`, sources `source/onde_boreale_glace_v2/`, aperçu `apercu_onde_boreale_glace_v2.html`. Le commit `dfa2ac83` contenait déjà les bonnes sources ciel/nuages et les trois entrées glacées : ne pas les régénérer. 39 fichiers V1 repris byte-identiques (30 plans terrain jour/nuit, 6 climat, 3 scènes jour). Nouvelle onde générée seule depuis `aurorepmdsky.png` : trois essais magenta conservés ; le troisième `onde_pmd_magenta.png` retenu, les deux premiers rejetés (tube épais puis effet de pont). Un dessin maître, pas 32 dessins générés.

Détourage, alpha de luminance, normalisation uniforme nearest ; 32 couleurs × 8 groupes de phase = 256 indices fixes, 32 LUT interpolées en teinte HSV (pas permutation stricte V12), ondulation verticale maximale ±2 px, aucune translation horizontale. 32 frames ×125 ms =4 s, boucle exacte. 32 PNG d’effet +96 PNG de composition, huit WebP et trois ORA nocturnes à neuf plans. PNG/WebP visibles via les README GitHub, pas uniquement une archive. Les boucles WebP4s gardent les nuages fixes ; les extraits8s déplacent réellement les nuages à−4px/s, loop=1 (une lecture), pas une fausse boucle courte. Wrap natif360s indépendant dans le viewer. Dessin généré référencé, pas cycle officiel extrait. 41 tests d’assets et21 tests de galerie DOM simulé PASS, anciennes suites60/140 PASS ; aucun navigateur réel, GPU, collision ou runtime PMDO revendiqué. Candidat non encore approuvé par l’utilisateur.


## Arène de glace avec montagnes Sky Peak et lune canonique

Demande suivante : une arène de glace, montagnes enneigées lointaines façon Sky Peak, ciel bleu-noir étoilé, lune canonique. Nouvelle livraison non destructive `renders/arene_glace_sky_peak_v1/` ; sources du même nom, aperçu racine `apercu_arene_glace_sky_peak_v1.html`. Trois générations : terrain sur magenta avec accès sud, panorama montagneux sur magenta, sol sous-jacent complet. Références réelles IceArena/IceRoad et SkyPeak232233/sommet. Terrain normalisé uniformément960×644, posé(0,140) sur960×720, bas recadré ; grade nocturne choisi RGB×(.72,.80,.96), pas Abyss ni palette native certifiée. Panorama960×475 en(0,45). Huit plans : ciel, étoiles, lune, montagnes, sol complet, relief arrière, immersion gauche/droite. PNG, ORA, planche, galerie, ZIP compact.

Lune : plus grande composante8connexe de la feuille `source__falaise__astres_nuit_native.png` c16efe12, ROI(312,24)-(368,80),33×36,516pixels visibles, posée(800,64), RGBA exacts sans resampling/recoloration. Étoiles par recette existante. Ciel :65premières lignes sombres de climate.sky conservées, puis couleur existante dominante de la ligne64 prolongée uniformément. Ne pas dire que tout le ciel est inchangé : turquoise exclu pour le bleu-noir demandé. La répétition de la ligne bruitée créait des stries verticales, essai rejeté. Scène fixe, sans aurore/nuages ajoutés. Sol caché généré, faces cachées des reliefs non reconstituées.29contrôlesPASS dont lune native, recompositionsPNG/ORA et corridor16px ; syntaxeJS/liens/ZIP contrôlés, pas navigateur réel ni runtime/GPU/collisions. Candidat à examiner par l’utilisateur.

Sandbox retrouvée au commit de base avec ancien lot falaises non commité ; état sauvegardé dans un stash nommé avant fast-forward de cette même branche vers ca2ed3fc. Stash conservé, ne pas le réappliquer aveuglément sur les ciels corrigés.


## Correction — aurore couvrant les deux côtés du ciel de l’arène, sapins enneigés V2

Dernière précision utilisateur : « l’onde boreal prends le manque au ciel sur chaque côté ». Complément `renders/arene_glace_sky_peak_v2/`, sources du même nom, aperçu `apercu_arene_glace_sky_peak_v2.html`. Huit plans de V1 `d972a438` conservés byte-identiques : terrain/accès sud/bordures, montagnes, ciel bleu-noir, étoiles et lune native. Deux nouvelles générations : aurore panoramique sur magenta (références ondeV2+compositionV1+aurorepmdsky), sapins enneigés (snow.png canonique+compositionV1). Aucun nouveau terrain généré.

Aurore1808×592→1056×346nearest uniforme, pose(-48,-8), débordement latéral ; pas de miroir/répétition ni fondu à zéro aux bords du cadre. Alpha de luminance,32couleurs×8groupes de phase,32LUTinterpolées ; ondulation verticale±2px sans scroll/wrap.32frames125ms=4s, boucle exacte. Sapins recadrés puis640×170en(160,175), derrière relief, devant montagnes. Lune au-dessus de l’aurore, native33×36 à(800,64), pixels opaques préservés dans toutes les scènes. Couverture visible après occlusion contrôlée dans les bandes latérales120×300 : minimum18373pixelsgauche/21728droite ; les colonnes extrêmes restent éclairées à toutes les phases.

Dix plans960×720,32PNGd’effet,32PNGdescène,troisWebP,ORA,plancheavant/après et cataloguesGitHub.23testsassets+13testsDOMsimuléPASS,29testsV1PASS. Pas navigateur réel/runtime/collisions/warp ; nouveaux dessins référencés, pas cycle officiel ni textures natives certifiées. Pas de nouveaux nuages ajoutés à l’arène. L’ancien ZIPV1 ne contient pas les ajouts ; nouveaux fichiers accessibles dans le dossierV2.

Lors de cette reprise, checkout restauré àe387ae7b avec anciennes falaises non commitées : stash nommé conservé, puis fast-forward de la même branche versd972a438. Ne pas réappliquer ce stash périmé sur les ciels corrigés.


## Correction suivante — arène V3 et animation ciblée du Ground utilisateur

L’utilisateur autorise la régénération de l’arène pour retrouver roche/glace des références IceArena/IceRoad, demande les montagnes **hyper loin** et une **mer d’arbres enneigés dans la vallée entre montagnes et arène sur son sommet de falaise**. Il demande séparément d’animer nuages en overlay et mer de `cliffnordouesttest1.rsground`, **sans rien toucher d’autre**.

`renders/arene_glace_sky_peak_v3/` : nouveau terrain, frise de montagnes, forêt profonde générés depuis les références. Quatre bruts conservés (premier bandeau forestier remplacé car son bas laissait du vide). Canevas960×896 ; terrain960×644 à(0,270), forêt à(0,213), montagnes à(0,160). Détourage renforcé contre franges magenta ; pas de recoloration post-génération. Dix calques PNG, quatre poses de composition, WebP32phases, ORA. Ciel/étoiles/aurore/lune V2 gardent leurs pixels et positions dans leur emprise960×720 ; extension du canevas vers le bas uniquement. Les32PNG d’aurore restent référencés dans V2 pour éviter les copies. Sol/roches = partition exacte des pixels visibles, **pas de sol caché reconstitué**.17testsassets+13testsDOMsimuléPASS ; pas de Ground/GPU pour l’arène. Dessins référencés, ne pas revendiquer des textures natives copiées ou une approbation utilisateur.

`exports/cliffnordouesttest1_animation_v1/` : **PARTIEL, nuages seulement ; mer inchangée**. Root Ground patché uniquement par ajout de `Object.Status.cliffnw_native_cloud_overlay`, sans resérialisation globale. Tous les autres octets, BOM/CRLF inclus, identiques à l’original épinglé `aac14ae4`. Original disponible dans Git ; constructeur refuse d’écraser des edits indépendants. Quatre calques, toutes les frames et cadences, obstacles, entités, décorations, fond, métadonnées préservés.

Émetteur natif `OverlayEmitter`, coucheTop4, six familles Guilde/Sharpedo de `ciels_valides.py`, texture1440×784 ày208, −4px/s, wrap360s. Deux dépendances à installer avec le Ground : `Content/BG/CLIFFNW_NATIVE_CLOUD_OVERLAY.dir` et `Data/MapStatus/cliffnw_native_cloud_overlay.json`. Statut masqué, sans états météo ni événements gameplay. Démarrage en entrée de Ground via `GSceneZone.InitGround`, pas par remplacement de son Background (qui serait masqué par le ciel en tuiles).

14tests de préservationPASS et **13tests natifs dans le vrai PMDO0.8.12PASS**. Runtime réinstallé en cache depuisRUNTIMEPMDO (archiveSHA256c64f72af…61577) +DumpAsset épinglé, hookLua temporaire restauré ; chargement Ground/MapStatusData réel, type/vitesse/couche vérifiés. Pas de GPU, de lecture animée ou de chargement des banques absentes validé. Test initial enumLua corrigé avec la constante nativeTop puis relancé. Cache non versionné.

**Pour terminer la mer, obtenir les banques exactes `v2_promontoire_jour_03.tile`, `terrain.tile`, `terrain (2).tile`, `terrain (3).tile`, `terrain (4).tile` de Content/Tile.** Non présentes dans le checkout/archives ni retrouvées dans les arbres de dépôts inspectés. Le Ground ne contient pas leurs pixels. Ne pas inventer une zone d’eau d’après son nom ou animer la cascade Metano à la place. `00_ciel` examiné est une texture de ciel, pas une preuve de mer. La carte ne doit subir aucun relayout.


## Ajout utilisateur — mode nuit du Ground, sans changer sa géométrie

Nouvelle précision « et aussi que tu le mette en mode nuit » interprétée comme visant `cliffnordouesttest1.rsground` ; arèneV3 déjà nocturne, aucun de ses assets régénéré ou modifié. HEAD distant vérifié identique à34d40dc0 avant intervention ; banques personnalisées toujours absentes.

Nouvelle livraison `exports/cliffnordouesttest1_nuit_v2/`, sources du même nom. Root Ground désormais nocturne, seulementObject.Status changé par rapport àaac14ae4. Statutunique `cliffnw_night_overlay` / MultiSwitchEmitter ordonné : voile RGB(8,14,36) alpha176 enTop puis six familles de nuages nocturnes Guilde/Sharpedo, même alpha/géométrie1440×784 ày208, −4px/s. BGvoile natif1×1 avec casplein-écran deOverlayAnim, pas unPNGtileset. Le voile éclaire toute la scène, personnages compris : **pas une conversion canonique des palettes des banques**. Pas de nouveau ciel/lune/étoile ni script. Original et versionjour conservés dansGit/dossierV1 ; ancienstatutjour non cumulé avecnuit. Quatre fichiers à installer ensemble :Ground,MapStatus etdeuxBG.

20contrôles de préservation PASS,21tests natifs PMDO0.8.12 PASS. Le vraiGround etMapStatusData sont chargés ; cycle de vie du vraiMultiSwitchEmitter cloné testé dans unTitleScene(false) natif (listesBaseScene, sansBegin/Draw), ordre, deuxanimationsuniques,temps etarrêt vérifiés. Premier essai GroundScene() a échoué parce que sonconstructeur crée unRenderTarget2D ; fichiers restaurés et suite relancée avec le conteneur sansdessin. **Aucun testGPU/lectureenjeu/éditeur**. Source/runtime_verification.json détaille ce périmètre. VérificateurV1 adapté à sonGroundjourarchivé, pas aurootnuit.

**Mer toujours non animée** ; le voile la fonce visuellement mais aucune de sesframes ne change. Obtenir `v2_promontoire_jour_03.tile` et les quatre `terrain*.tile` exacts. Ne pas fabriquer un rendu complet de la carte avec des banques supposées. PNG/WebP fournis = effetsseuls, pas aperçu completGround.
