...
## Nouvelle zone glaciaire — grotte nord et aurore multicouche

L’utilisateur demande une nouvelle zone glaciale avec **falaise/grotte de glace au nord**, chemin lisible et continu venant du **sud**, et ciel boréal animé. Livraison : `source/grotte_glaciaire_boreale_v1/`, `renders/grotte_glaciaire_boreale_v1/`, ZIP `renders/grotte_glaciaire_boreale_v1_pack.zip`, aperçu `apercu_grotte_glaciaire_boreale_v1.html`.

Méthode Guilde appliquée : trois réponses générées séparément (terrain complet sur magenta, ciel sans aurore, planche d’aurores 2×4 sur magenta), détourage par inondation depuis les bords, puis calques alignés de neige/chemin/vide de grotte/falaise nord/deux flancs/premier plan. Ne pas remplacer ce rendu par une mosaïque de morceaux de map. Terrain et ciel sont fixes ; l’aurore transparente est une animation **en place** de 16 phases ×120ms (1,92s), sans scroll ni wrap, avec ORA, masques, GIF/WebP, recette et viewer.

La planche d’aurore a été auditée : seules les poses 0–3 (famille de rubans/ancrage cohérents) sont utilisées, avec trois intercalaires alpha-prémultipliés entre chaque paire. Les poses 4–7 sont archivées dans le brut, pas présentées comme un cycle propre. Cycle proposé et images générées ≠ cycle officiel PMD récupéré ni pixels canoniques. Les 11 tests dédiés valident les hashes/bruts, alpha magenta, partition/recomposition, continuité géométrique sud→seuil nord (largeur visuelle minimale71px), boucle, ORA, GIF/WebP et syntaxe viewer. **Pas de Ground/tileset/collision/warp/parallax/import/GPU/runtime PMDO ou approbation artistique déclarés.**
