...
### Méthode confirmée pour les prochaines maps — Entrée Grotte des Cascades (26 septembre)

L’utilisateur confirme de réserver la méthode aux **nouvelles maps**, sans reprendre les anciennes livraisons. Il demande « comme l’agent avec les lots d’il y a une heure » : composition complète au générateur sur fond magenta, sol complet séparé, calques alignés, exports PNG 8 px et Ground PMDO ; **pas d’assemblage de fragments de maps**. Si la référence ou l’interprétation de « texture canonique » est ambiguë, poser la question avant le build. La génération en DA PMD ne doit pas être décrite comme des pixels de tiles natives certifiés.

Première carte selon cette consigne : `source/entree_grotte_cascade_sud_nord_v1/`, référence jusque-là inutilisée `Waterfall_Cave_ledge_TDS.png`, rendus `renders/entree_grotte_cascade_sud_nord_v1/`, aperçu `apercu_entree_grotte_cascade_sud_nord_v1.html`. Format 4:3 : 768×576, 96×72 cases de 8 px ; progression sud→nord. Décor, sous-couche et matière d’eau tous générés sur magenta; normalisation uniforme 1200×896 → 768×576 par BOX classé. Pas de pixel de la référence assemblé. Eau générée, 12 phases × 10 ticks, palette limitée, masque fixe; cycle/cadence créés pour le lot, non officiels. 6 calques Ground, marqueurs `entrance` et `donjon_seuil`, aucun warp. Neuf tests d’assets/round-trip PASS, dont un contrôle explicite qui rejette une case de départ bloquée ; **pas d’approbation artistique, pas de test PMDO en jeu ou runtime**. La `.venv` locale a été recréée avec Pillow 12.3, NumPy 2.4 et SciPy 1.17 (ignorée par Git).
