...
### Entrée Cascade V1 — retour aux pixels natifs exacts en 4:3 (26 septembre)

Utilisateur : « oui lance toi ! » (map suivante, Waterfall Cave), puis interruption après le premier brut généré : **« tu as de l'idée dans tes générations mais je demande les textures canoniques stp ! »**. Règle retenue : quand l'utilisateur dit « textures canoniques », la map se construit en **pixels natifs exacts** (méthode `zones_south_north_v3`), le rendu généré ne peut servir que de **guide de composition** et aucun de ses pixels n'entre dans les exports ; le dire explicitement dans README, manifest et aperçu. Lot `source/entree_cascade_sud_nord_v1/`, aperçu `apercu_entree_cascade_sud_nord_v1.html`.

Outils du lot (réutilisables) : classe `Canvas` (RGBA + provenance `pid/sx/sy` par pixel, `put` avec masque et couture verticale `seam_v` à coût minimal, `min_path` en programmation dynamique), `dark_cut` (coupe le bas d'un bloc le long d'un chemin de pixels sombres = contours natifs, pour poser des parois sur le sol sans bord droit), `quilt` (Efros–Freeman simplifié : blocs natifs, choix parmi les 3 meilleurs candidats par coût de chevauchement, coutures gauche/haut ; option `phase` pour respecter une période verticale native — le couloir de Waterfall Cave a une période de 24 px, ce qui rend les coutures du sol quasi invisibles ; l'eau en mailles n'a pas de période, les coutures restent acceptables). Objets masqués : rochers de G = tout sauf `r >= g` (pourpre), eau (`b − r >= 100`) et noir ; cristaux = saturation > 120 ou luminance > 150 ou socle gris clair ; source turquoise = teinte turquoise + liseré blanc, trous remplis ; pierres = blob sombre dilaté de 1 px. Les parois de L sont posées à l'identité (gauche) et à +360 px (droite) et le couloir natif de L est posé aux mêmes offsets pour un raccord sans couture ; les pièces du plafond alternent A/B (colonnes hors couloir) avec chevauchement de 24 px. La cascade Métano est posée par-dessus la rangée nord entre deux paires de rochers (calque rochers au-dessus de la cascade), le pied sur la plateforme-seuil ; les pierres de liseré sont placées côté eau pour ne pas bloquer la chaussée. Pillow fusionne les frames WebP identiques : exporter une frame par phase (durée 10 ticks), pas un pas de 5 ticks. 10 tests PASS (dont relecture pixel à pixel des sources via `provenance.npz`). Pas de runtime.

### Entrée Cascade V2 — correction : « textures canoniques » = générateur RÉFÉRENCÉ par les rips (26 septembre)

Utilisateur : « tu dois utiliser ton générateur d'image tu as mal audité l'ancienne méthode ». **Règle corrigée** : dans la série des entrées sud → nord, « textures canoniques » signifie que le générateur reçoit le rip canonique du biome **en image de référence** (`images=[rip]`) et reproduit ses textures (cf. Bristle : brut ≈ rip Mt. Bristle). Le premier brut Cascade (V1) avait été généré sans cette fidélité, d'où l'interruption « je demande les textures canoniques » — qui ne demandait PAS un relayout de pixels natifs. Ne plus interpréter « canonique » comme « pixels natifs exacts » dans ce contexte ; la méthode native (`zones_south_north_v3`, `entree_cascade_sud_nord_v1`) reste disponible seulement si l'utilisateur la nomme explicitement. La V1 native est conservée (non destructif), la V2 est le livrable attendu : `source/entree_cascade_sud_nord_v2/`, aperçu `apercu_entree_cascade_sud_nord_v2.html`.

Points techniques : le générateur avec deux rips en référence a rendu un décor 1200 × 896 fidèle du premier coup (stalactites, rochers bandés, galets, cristaux, eau en magenta). Il n'a pas su produire un « sol complet » propre (deux essais gardaient rochers et contours des bassins) : le sol complet est édité par **quilting de blocs 96 px du sol du décor** (`quilt_full`, ~5 min en pleine résolution, fait une fois, brut hashé). Segmentation mesurée : sol = (b − r lissé > 52) & (lum lissée > 92) relié au bord sud (le sol est nettement plus bleu que les rochers, b − r 67 contre 45) ; cascade = magenta relié au bord haut. Les cristaux disparaissent dans la palette commune de 96 couleurs : les quantifier à part. Cascade native tuilée : garder les lignes 66–101 (motif ^^^ régulier) + pied 102–119 ; les lignes 38–60 contiennent des bandes de crête qui font un effet d'échelle une fois répétées. Le liseré clair (247,255,255) n'existe pas dans les bassins du rip : (215,215,215) oui — le test exige que la palette de l'eau soit incluse dans les couleurs du rip. Le pipeline Jungle est réutilisé par `loadmod` (même géométrie 1200 × 896 → 768 × 576) avec `JM.PAL` remplacé. 9 tests PASS, pas de runtime.
