...
## Reprise maps — entree aride generee V2 (22 septembre 2026)
...
Variante frontale (falaise + bouche proche, 400x360) : `source/aride_generee_v2/`
(build/package/tests/STATUS), `renders/aride_generee_v2/` +
`renders/aride_generee_v2_pack.zip`, `apercu_aride_generee_v2.html`. 4 bruts
(parois/bouche a gauche, sol+sentier, 8 props isoles, 3 volutes), downscale /3,
detourage magenta seuil global d<170 + pelage (le seuil serre V16 laissait la
frange cuite d100-230), 12 couches, FX 12x100ms boucle parfaite, ORA/GIF/WebP,
8 tests PASS. Sentier sud->seuil avec degagement 8 px.
Note : la V1 (`aride_generee_v1`, variante couloir sinueux vers grotte lointaine)
a ete poussee par une session parallele (8f597163) et est conservee telle quelle.
