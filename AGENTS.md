...
## Nouvelle zone demandée — grotte glaciale, chemin sud→nord et ciel boréal

L’utilisateur demande ensuite une **nouvelle zone glaciale** : falaise/grotte naturelle de glace au nord, arrivée et chemin lisible depuis le sud, ciel boréal animé. Livraison V1 : `renders/zone_glaciale_grotte_boreale_v1/`, ZIP voisin, sources/rebuild dans `source/zone_glaciale_grotte_boreale_v1/`, aperçu autonome `apercu_zone_glaciale_grotte_boreale_v1.html`.

Méthode Guilde maintenue : trois compositions générées séparément (terrain et aurore sur magenta, ciel opaque), bruts/hashes conservés ; matte reproductible ; calques alignés (ciel, étoiles, aurore, sol sous relief, vallée, chemin, falaise nord, falaises latérales, cadre/profondeur de grotte, détails) et ORA. Le terrain n’est **pas** un assemblage de fragments de map ni un tileset natif : ses pixels générés sont conservés à leur taille et simplement translatés de 200 px pour révéler le ciel. Le sous-sol est une couche d’édition générée et masquée dans la recomposition, pas une reconstruction du volume caché.

L’aurore a 16 frames à 100 ms (1,6 s), chacune transparente et alignée sur 1 264 × 1 008. Son dessin indexé/alpha et ses palettes sont livrés. Mouvement = onde transversale verticale par colonne + rampes cyan/violet-rose cyclées, sans scroll/wrap ; frame 16 = frame 0 contrôlée. Le chemin géométrique du sud `(632,1007)` au seuil `(632,548)` est continu, mais ne vaut ni collision, ni warp, ni test PMDO. `verify.py` produit 53 PASS : dimensions/grille, provenance, pixels de terrain conservés, partition/recomposition exacte, route, animation, boucle, magenta, formats, ORA et syntaxe viewer. **Art, import, occlusion de personnage et runtime PMDO restent non approuvés/non testés.** Attendre un retour visuel avant de remplacer ou régénérer ce candidat.
