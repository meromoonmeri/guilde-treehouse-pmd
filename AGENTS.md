...
## Audit du 24 septembre 2026 — générations multicalques NON CONFORMES

L'utilisateur a signalé un problème de qualité. L'audit `audits/multicalques_v1/RAPPORT.md` le confirme : **0/51 zones multicalques générées passent le contrôle, contre 3/3 références canoniques.** Défauts : faux pixel art venant directement du générateur (13 000 à 245 000 couleurs par calque, contre moins de 810 dans le canon), réductions LANCZOS floues, clé chroma lue sur le pixel (0,0) qui laisse jusqu'à 362 000 px magenta, calques générés indépendamment et incohérents, calques vides, copies du composite, tailles hors grille 8 px.
Règles impératives : ne jamais livrer des pixels du générateur comme pixels de map (le générateur sert de guide de composition) ; pas de rééchantillonnage LANCZOS, BILINEAR ou BICUBIC ; clé magenta explicite et alpha binaire ; un calque = un contenu distinct et non vide ; composite = empilement exact. **Exécuter `.venv/bin/python source/controle_qualite_pixel/gate.py <zone>` et obtenir PASS avant toute livraison.** PASS ≠ validation artistique ni test PMDO. Ne pas corriger à la marge les 51 zones déclassées ; les reprendre par assemblage natif. Relic Forest et Vast Steppe ont des palettes d'herbe incompatibles (79,135,95 contre 127,199,95) : ne pas les mélanger sans transition native.
