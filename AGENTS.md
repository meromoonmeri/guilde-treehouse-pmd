...
## Reprise du 22 septembre 2026 — « spriter pro, maps aux textures canoniques »
...
## Jardin secret v1 (24/09/2026) — layout agrandi aux textures de la référence
- Demande : « layout plus grand pour secret garden avec les textures canoniques de la référence, bordure de feuille immersive ». Interprété : matière = pixels de `secretgarden.png` UNIQUEMENT (pas Métano/Halcyon, qui ont un autre style).
- Méthode nouvelle et réutilisable pour toute référence PMD Sky aplatie : `source/jardin_secret_v1/quilt.py` (quilting guidé FFT, coupe minimale, pixels imposés, provenance `src` y,x). Guidage = distances signées aux bords + distance « depuis le bord nord » + ton du tapis en 4 bandes mesurées. Sans ces caractéristiques directionnelles, l'ombrage des bords nord et les lignes de transition sont recopiés en plein milieu.
- Leçons : (1) la référence n'a pas de bord orienté sud → faire « couler » le plan vers le sud et cacher les fermetures sous le feuillage d'avant-plan ; (2) le halo du rayon (verts b=55, colonnes x 127–130 / 276–280) contamine sprites et patchs → l'exclure des sources ; feuillage des arbres = b 31 ; (3) pénalité de réutilisation sinon touffes d'herbe en grille ; (4) feuillage d'avant-plan = synthèse de la frange sombre puis silhouette g ≤ 151 sur pelouse / g ≤ 119 sur sous-bois ; (5) `binary_erosion(..., border_value=1)` pour les tests de parcours au bord.
- Livré : 8 calques `JSEC_V1_*` jour/nuit, ORA, `verification.json` all_pass (pixels = source, aucune couleur nouvelle, parcours sud → escalier). Aucun test PMDO. Les précédents lots sont inchangés.

## Jardin secret v2 (24/09/2026) — méthode spriter magenta
- Demande : « méthode sprite artiste / pixel artiste humain, générateur fond magenta, assemblage multicalque, correction avec outils » + « chemin droit, bordure de feuilles immersive, arbres à la taille canonique Halcyon, fleurs animées sur leur propre calque, petit temple japonais miniature pour Celebi sur la souche ».
- Sources `source/jardin_secret_v2/` : maquette.py → bruts (générateur) → extract_v2.py → compose_v2.py → export_v2.py. v1 (quilting pixels natifs) conservée intacte.
- Leçons : (1) le générateur ignore souvent la grille demandée (planche 3 arbres rendue en 3×3) → découper par bandes vides mesurées, pas par tiers ; (2) il déborde des masques de feuillage → recouper sur la maquette avec bord FESTONNÉ (une coupe lisse se voit) ; (3) échelle = mesure sur l'objet (largeur de cime 126 px Halcyon), pas le facteur d'image ; (4) masques de parcours : fermer/boucher les touffes sombres du tapis sinon faux échec ; (5) read_file garde en cache un chemin déjà lu → copier sous un nouveau nom pour revoir un rendu régénéré ; (6) .venv non persistée entre tours : la recréer.
- Fleurs : 3 horloges 8/10/14 gf × poses 0,1,0,2, base lignes 14–23 = pose 0. Temple/fleurs en palette propre (rouge/jaune absents de secretgarden.png) ; reste en palette de la référence.
- v2b (retour « le rendu ne va pas », sans précision) : trois défauts identifiés et corrigés. (1) Bandes sombres vides : le feuillage est régénéré sur le masque exact du sous-bois, calculé depuis le sol GÉNÉRÉ et non depuis la maquette. (2) Arbres : ils tiennent dans le cadre 144×120, pas seulement à la largeur de cime. (3) Souche à 128 px pour un hokora lisible. Avec 3 images d'entrée, le générateur a changé le format (768×1376) ; avec 2 images, il reste en 864×1220.
