...
## Entrée Crooked v1 (24/09/2026) — map ⇒ layers de génération + natifs

- Demande : auditer si la méthode existe (OUI : layouts_magenta_v1/WORKFLOW + crooked_verdoyant V1/V2/V3), puis entrée grotte texture Crooked + arbres Halcyon + herbe/fleurs Sky Peak.
...
## Entrée Crooked FINALE (24/09/2026) — map générée puis décomposée en calques

- Demande : générer la map finale améliorée d'un bloc (guide V1 + refs canoniques), puis la décomposer en calques.
- Livré : `source/entree_crooked_finale/` (bruts/FINALE_map.png, build/verify/make_gallery), `renders/entree_crooked_finale/` (8 calques jour/nuit + 00_sol_complet, composites, manifeste, 17 PASS), galerie `apercu_entree_crooked_finale.html`. Canevas 848×1264, nuit Abyss exacte.
- Détection : bouche = plus grande composante sombre du haut (lum<70) ; arbres = cœurs vert sombre (lum<95 & g>r+15, >800px) + dilatation géodésique 5 it (lum<160) + fill_holes vert + garde des 4 comps >5000 ; troncs = brun bas du ROI comps >80 ; fleurs = blanc (lum>195 & sat<50) + crème, hors sable franc, y≥620 (65 comps 7–61px) ; chemin = sable (lum>170 & r>b+20) comps >200 + sol de bouche, corridor évasé en profondeur, hors boîtes cœurs ; rochers = comps chaudes 150–60000 au voisinage de la bouche (6) ; restes 2,3 % (vert→sol, sable-ish bas→chemin, sinon paroi).
- Leçons : (1) reconfirmé : jamais deux edits parallèles du même fichier — perte silencieuse, passer en script de patch séquentiel avec asserts ; (2) canopée : la dilatation géodésique dans la prairie inonde tout le ROI — contraindre lum + peu d'itérations + filtre de taille ; (3) les « roses » du générateur sont crème (aucun vrai rose, 9 px) ; (4) berge sableuse et reflets de paroi exigent des garde-fous spatiaux (corridor, voisinage bouche, y≥620).
