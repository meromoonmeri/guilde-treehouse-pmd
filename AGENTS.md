...
## Mt. Thunder orage V1 — multicalque, nuages et éclairs animés

Demande : layout multicalque légèrement différent de la référence Mt. Thunder (PMD Rouge, boss room), mêmes textures/palette, nuages et éclairs sur leurs propres calques animés. Aucun générateur : pixels 100 % référence (0 couleur hors référence testée). Plateau élargi 32 px (bande centrale dupliquée), décors en position symétrique sur calque séparé, pied prolongé sous les nuages avant. Nuages : bandes 864 px original+miroir, RepeatX -8/-16 px/s. Éclairs : planche 1-4 + Flash, Normal/Fading exacts, droite en miroir ; séquence 30 frames/7,4 s = choix. Piège : la bande avant doit être uniquement le blanc relié au bas de carte (les gris 192 latéraux sont derrière le plateau). Sources `source/mt_thunder_orage_v1/`, rendus/ZIP `renders/mt_thunder_orage_v1*`, aperçu `apercu_mt_thunder_orage_v1.html`. Non validé en jeu ni par l'utilisateur.
...
## Chemins droits (V3c, V4b), correctif des collisions, sommet coucher V5

V3c : le guide sans rochers est édité au générateur pour avoir un chemin droit vers la grotte. V4b : la fissure en zigzag est retirée (sol natif ±48 px) et un chemin droit est tracé avec les jaunes canoniques (intérieur 232,216,88, bords 200,184,48). Le jaune clair seul ne se voit pas. Correctif : dans V2/V3/V3b, les collisions choisissaient le mauvais composant (le chemin beige n'était pas jaune) ; le sol marchable = herbe jaune + chemin beige, plus grand composant, et ces lots ont été régénérés. Attention : read_file affiche parfois une image périmée, vérifier la grille dans le manifest. V5 : planche `112438.png` (soleil 12 frames), pile recomposée à 99,9 %, élargissement de 64 px. Aperçus `apercu_thunder_meadow_v3c_entree.html`, `apercu_thunder_meadow_v4b_sommet.html`, `apercu_summit_coucher_v5.html`. Non validés en jeu.
