...
## Harnais de tests — commande unique et clones shallow (18 septembre 2026)

État vérifié de l'ensemble du dépôt : **`238 passed, 3 skipped`** en ~30 s via `.venv/bin/python -m pytest`, depuis une `.venv` reconstruite par `pip install -r source/requirements.txt`. `pytest` est ajouté à ce fichier de dépendances et `pytest.ini` fixe `--import-mode=importlib` + `consider_namespace_packages` : les 9 lots « boréales / arène de glace » font `from . import build as b` (import relatif) alors que les autres importent `build` en module de premier niveau, et beaucoup de fichiers s'appellent `test_build.py`, ce qui cassait toute collecte groupée en mode `prepend` (« import file mismatch »). Ne pas supprimer `pytest.ini` ni réintroduire un lanceur maison : les deux conventions doivent rester exécutables dans la même commande.

Les 3 `skipped` ne sont pas des contrôles perdus. `zones_relayout_v1`, `zones_relayout_v2` et `zones_south_north_v3` vérifient d'abord le SHA-256 des originaux utilisateur contre leur manifeste (toujours exécuté), puis les comparent aux blobs épinglés `9ec9a081` / `438b9288`. Les sandboxes Arena sont des **clones shallow** (`.git/shallow` ne liste que 2 commits) : ces objets y sont absents et `git show` échouait alors que les fichiers étaient intacts. `source/git_provenance.py` (`commit_available`, `pinned_blob`) sépare « fichier modifié » de « historique indisponible ici ». Le commit `cda5661b` (session précédente) tolérait déjà ce cas dans `zones_south_north_v3` en sautant la comparaison en silence ; les trois lots passent maintenant par l'aide partagée et signalent l'écart par `skipTest`, donc le trou reste visible au lieu d'être avalé. La comparaison réelle s'exécute dès que l'objet existe — vérifié : `pinned_blob('HEAD','README.md')` renvoie exactement les 27524 octets du working tree, et `commit_available('9ec9a081')` renvoie `False`. Ne pas transformer ce skip en passe silencieux.

Toujours `git fetch` la branche de session avant de conclure sur l'état du dépôt : `cda5661b` (test de décodage des assets indexés V12 + docs du lot) était déjà poussé alors que le checkout local restait sur `e387ae7b`, ce qui faisait apparaître comme non corrigé un problème déjà traité.

Contrôles historiques : **13 des 14** scripts `source/verify_*.py` + `audit_zones_metano.py` sortent OK/PASS (guilde `verify_pmd.py` : 12 salles, 24 Aseprite, 11 calques, 6 vues, magenta exact, Tiled identique). Seul `verify_browser.py` échoue, faute de `playwright` (dépendance optionnelle commentée dans `requirements.txt`, non installée ici) — pas une régression, et toujours pas une validation de rendu moteur.

**Bug corrigé au passage — `audit_zones_metano.py` détruisait ses propres mesures.** Son docstring annonce « read-only » mais il réécrit `audits/metano_import/mesures.json`, et sa Ground de référence `/home/user/metano_town_palika.rsground` vit **hors dépôt** : absente dans une sandbox neuve, le script remplaçait le bloc `original_ground` (blob `38d17852…`, TexSize 1, Base 35646 cellules, Cliffs 2169, toutes en coordonnées source→map identiques) par `{"available": false}`, effaçant la seule mesure d'échelle native du rapport d'audit. Désormais, référence absente ⇒ mesures enregistrées **reportées telles quelles** avec `re_measured: false` et une note explicite ; `available: false` n'est écrit que si aucune mesure n'a jamais été consignée. Vérifié : après restauration du fichier versionné puis relance, le diff n'ajoute que ces deux clés et la planche `comparaison_echelle.png` reste byte-identique. Ne pas réintroduire l'écrasement, et ne pas faire passer ce report pour une nouvelle mesure.

## Priorité guilde choisie — batch V7 : Pain et DeepBreath sans anatomie inventée (18 septembre 2026)

L'utilisateur a choisi de reprendre **les animations de guilde manquantes** (124 au registre), dans l'ordre déjà fixé : manques des neuf membres, puis Team Dazzling, puis transition de combat, puis retour aux maps (méthode « composition générée complète » retenue pour ces dernières).

Premier lot `source/guild_scene_actions_v7/`, exports `exports/guild_scene_actions_v7/`, galerie `apercu_guilde_actions_v7.html` : **Pain** (feuille native `Hurt`, 8 directions) et **DeepBreath** (feuille native `Idle`, 8 directions) pour les six membres incomplets, soit 12 cycles. Chaque frame produite est une **copie translatée** d'une frame native épinglée : aucune anatomie inventée, aucun recoloration, aucun masquage d'accessoire — c'est pourquoi ce lot commence par les seules familles d'actions sûres à ce titre. Bagon/Ptiravi/Pachirisu (profil natif complet) sont intacts.

La preuve de provenance est reprise du standard des zones : `test_build.py` rejoue la translation depuis `verification.json` et exige l'égalité pixel exacte avec la source native, la fermeture des boucles et l'alpha binaire. Précontrôle `validate.py` (dungeon) PASS par pack ; crédits copiés hors des dossiers d'export. **Pain n'est pas un renommage de Hurt** : chorégraphie distincte (flinch maintenu, tassement, retour) sur les pixels natifs ; déclaré stade technique, pas approbation artistique ni runtime.

Registres alignés : 124 → **112 actions sans cycle local** (`guild_eat_all_v6/production_progress.json`, `PROGRESS.md`, `FULL_PROGRAMME_STATUS.json`). Prochaine étape honnête de la même ligne : les familles qui changent l'anatomie (Sit, Laying, EventSleep/Wake vers la feuille Sleep native, famille KO) exigent un authoring par espèce avec masquage des accessoires — ne pas les produire par translation seule.

## Arène de glace V1 repassée au générateur — 5 calques + boréal animé (18 septembre 2026)

L'utilisateur a demandé de **repasser la zone au générateur pour la décomposer en plusieurs layouts dont le composite égale la référence** : terrain / bordure / cliff / sky / boréal, avec le boréal qui change de couleur et une légère ondulation. Lot `source/ice_arena_multicalques_v2/`, exports `exports/ice_arena_multicalques_v2/`, aperçu `apercu_arene_glace_calques_v2.html`.

Méthode retenue (conforme « le générateur fournit le guide, jamais les pixels ») : le générateur produit une carte sémantique plate (`generation/layer_masks_guide.png`, 5 couleurs index) ; elle sert seulement à affecter chaque pixel de la référence à un calque. Ciel/boréal séparés programmatiquement (aurore saturée), région de glace découpée terrain/bordure/cliff par la carte. Les calques portent donc les pixels de la référence et le composite recompose exactement `layout_guide.png` (`reconstruction_exact: true`) — la géométrie n'est pas régénérée. Le boréal est animé (cyclage de teinte + ondulation ≤2 px, boucle fermée, frame 0 = référence) : art généré, pas le cycle officiel.
