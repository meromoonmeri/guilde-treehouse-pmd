...
## Furnace Desert — biome eau V1 (25 septembre 2026)
Demande : « cette zone en biome swapping en eau, remplace tout le sable par de l’eau sur son propre calque et anime-le ainsi que le siphon ». Pièce jointe NON reçue dans le bac à sable (pas de `uploads/`, rien sur la branche) ; wiki injoignable (SSL) → scène pleine taille récupérée par recherche d’images : pamtre-berry « Furnace Desert, original version » 456×336 (rayons de soleil incrustés, absents de la version wiki de l’utilisateur ; miniature wiki 300×220 gardée pour comparaison). `source/furnace_desert_eau_v1/` : `segment.py` (couleur + silhouettes sombres remplies), `build.py`, `export.py`. Eau = tuile de haute mer native D25P11A doublement périodique (réseau (48,48)&(144,72), domaine 48×72, consensus ≈96 %), cycle réel 15 phases ×130 ms. Siphon = bandes par distance au bord de la cuvette + cycle de palette natif D14P11A (6 phases, vérifié couleur→couleur), bleus pris dans la palette D25. Chutes = texture de chevrons de la chute gauche, défilement 16 px/phase, boucle 96 px = 6 phases (le cycle de palette clignotait : abandonné). 1 077 px fondus des rayons (couleurs uniques) du calque roches ramenés à la couleur fréquente voisine (sinon gate FAIL 2,58 % de couleurs rares) ; ciel et premier plan exacts. `gate.py` copié depuis 01a0d30e (commit 15d07dd) dans `source/controle_qualite_pixel/` : PASS. Pas de pack `.rsground`, pas de test PMDO. Si l’utilisateur fournit son fichier exact : changer `REFERENCE` dans `segment.py` et relancer `export.py`.
Chantier Vast Steppe « lisière / grotte » EN PAUSE : `source/lisiere_fleurie_v1/README.md` (modules natifs, corniche cyclique, générateur configuré, question grotte/chemin sans réponse).
