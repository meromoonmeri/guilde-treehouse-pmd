...
## Spriter Pro — studio de cartes canoniques (20 septembre 2026)

Utilisateur : « j'ai besoin d'un spriter pro qui va me faire des map avec les texture canonique regarde tout les read me et outil etc ». Réponse : un studio réutilisable `source/spriter_pro/` (engine.py + layouts.py) qui prolonge la méthode approuvée de `source/build_metano_pixel_perfect.py`, plus un premier lot de six cartes `sprites/spriter_pro_v1/` (04_lac_suspendu → 09_trois_chutes), aperçu `apercu_spriter_pro_v1.html`.

Règles du studio : chaque cellule posée est une copie exacte d'une tuile native 8 px (Base, Cliffs, Animation_Tileset, River_Animation_1..4) ; primitives = rubans de parois, torrents nord→sud (réservoir/chutes/chenaux), bassins isolés (rangées natives du réservoir). validate_layout refuse toute pose non conforme (alignement 8 px, chutes ≥ 17 rangées sur front plat, torrent traversant toutes les parois, bandes sans recouvrement, bassins en plaine). Le vérificateur `source/verify_spriter_pro.py` est indépendant du build : blobs Git/SHA des sources, atlas octet à octet, white-lists par calque, recomposition des PNG à zéro différence. Ajouter une carte = éditer layouts.py puis build + verify. Ne pas présenter ces cartes comme testées en jeu ni intégrer au mod sans collisions/transitions. La tête de réservoir s'élargit avec l'offset du plateau nord (répétition de la rangée de surface) : garder des offsets ≥ 224 pour les grandes têtes.

## Plage PMD Sky — six propositions de layout (20 septembre 2026)

Utilisateur : « fait moi une map beachpmdsky propose plusieurs layout ». Six compositions générées dans la DA plage PMD Sky (ancrage `arenapmdskybeach.png` + planche TSR « Beach & Path to Beach ») dans `renders/beach_pmdsky_v1/`, manifeste et galerie `apercu_beach_pmdsky_v1.html` (script `source/build_beach_pmdsky_preview.py`). Propositions = compositions à choisir, pas des tilesets natifs ; après choix, reconstruction 8 px/calques si import moteur. La 04 rendue comme « descente à la mer » (pas d'embouchure visible) : nommer d'après le rendu réel, pas d'après le prompt.

## Plage canonique EoSO — layouts calques + eau animée native (20 septembre 2026)

Utilisateur : « je veux plus de layout de la référence que je viens de rajouter de cette plage la en plusieurs calque avec eau canoniquement animée ... regarde dans eos remake pmdo pour voir les animation de l'eau » (lien Minemaker0430/ExplorersOfSkyOrigins). Feuilles natives téléchargées et épinglées (commit bed94499) : D01P11A_layer1 (terrain 24px), D01P11A_layer2 (falaises/palmiers/herbe), beach_animation (eau : 7 bandes, 17 frames, FrameLength 16, pas +33 par frame ; la carte originale EoSO est un couloir vertical, nos layouts gardent la côte horizontale de la référence TSR). Studio `source/build_beach_canonique.py` + vérificateur indépendant `source/verify_beach_canonique.py` ; pack `sprites/beach_canonique_v1/` (6 layouts, calques back/eau×17/front, carte.tmj 3 calques, tsj natifs animés). Pièges rencontrés : le sable natif n'est pur que rangées 8-11 × colonnes 5-30 de layer1 (sinon bandes bleues/brunes) ; gid Tiled = firstgid + tileid 0-based (pas de +1) ; deux blocs qui se chevauchent dans le calque front cassent la règle « cellule = tuile native exacte » → interdire les chevauchements de blocs. Ne pas présenter comme testé en jeu.

## Plage canonique v2 — layouts recalés sur la référence utilisateur images.jfif (20 septembre 2026)

L'utilisateur a poussé `images.jfif` (scène TSR beach : mer nord, falaises cadrantes, pente de sable gauche, palmiers au milieu, rangée falaises/herbe au sud) et demandé « plus de layout de la référence que je viens de rajouter ». Convertie en `source/beach_eoso/reference_utilisateur.png`. Les six layouts canoniques ont été recomposés sur cette structure exacte (01 côte fidèle, 02 marée haute, 03 îlot au large, 04 plage abritée, 05 double rivage, 06 lagon clair) ; garde anti-chevauchement des blocs ajoutée au build (deux tuiles natives superposées dans le même calque cassent la règle de copie exacte). Rebase nécessaire avant push : l'upload utilisateur était arrivé sur la branche pendant le travail.
