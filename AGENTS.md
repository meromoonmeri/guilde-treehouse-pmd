...
## Revue des optimisations proposées pour `build_zones_guidees.py` (19 septembre 2026) — mesurées, non appliquées

L’utilisateur a soumis une note « réutilisation des textures PMD + 5 optimisations ». Revue dans `source/revue_optimisations_zones_guidees/` (`REVUE.md`, `mesure_optimisations.py`, `resultats/`), réplique en mémoire de la sélection, `sprites/` intact. Faits corrigés : le vocabulaire vient de **Métano** (feuille `Cliffs` = carte de la ville, 148 tuiles), pas des trois références PMD Sky ; les x 93–113 sont l’escalier et la porte de grotte, pas une « plage de transition » ; les intérieurs à ratio > .64 sont une **colonne unique 85/86/92 répétée** (tranche du module retour), pas un bloc 64 × 48. Mesures : poids de raccord .10 → .25 ≈ 0 % (terme ~50 × sous le masque, ≤ 22 candidats, 80–87 % des cellules sont des intérieurs hors score) ; 4 directions ICM −1,6 % ; hash sur [85,86,87,92] **+13/20 % de coutures et 90 % de paires non natives** ; run contigu 85…91 −19 % en intérieur (témoin seulement). Retenu pour les prochains lots : noms uniques par zone + contrôle `rglob` global (24 basenames PNG en double entre les deux zones). Ne pas réoptimiser l’assembleur cellule par cellule : la voie conforme reste les modules complets V4/V5 ; le niveau « rendu en jeu » reste manuel et non garanti.

## Plage Beach Cave grande — nouveau layout, cellules canoniques EoSO (20 septembre 2026)

Utilisateur : « j'aimerais un nouveau layout et les animation canonique de la mer et les texture canonique de beach cave entrance [dépôt EoSO], une plage plus grande etc ». Lot `source/plage_beach_cave_v1/` : références EoSO épinglées (`bed944992c32…`, blobs vérifiés en ligne dans `verify.py --online`), `SPEC.md` écrite avant l'assemblage, `analyse.py` (raccords), `layout.py` (plan de cellules), `build.py`, `verify.py` (79 PASS). Méthode : **ré-assemblage par cellules de 24 px** (TexSize 3), jamais de tuile redessinée : bloc de colonnes [11, 23) inséré une fois (raccord de boucle 204 < médiane 318), sable : rangée 9 répétée (raccords ≤ 92), côtés : la cellule qui se raccorde le mieux à elle-même (gauche rangée 8, droite rangée 9), **Back suit Front dans les colonnes latérales**. Piège rencontré : le calque Back porte les **ombres cuites** des détails Front (cailloux, buisson) — un détail se déplace avec ses cellules d'ombre, sur une cellule de même rangée d'origine et même colonne mod 8 (période du sable), sinon taches orphelines (vues au premier rendu). La mer garde ses 17 frames par cellule (315 cellules animées, 17 composites distincts, contrôle « pas de remplissage d'eau statique »). Livraison : `plage_beach_cave_v1_pmdo_0812.zip` (feuilles renommées `PLAGE_BC1_*`, octets identiques aux feuilles EoSO ; index propre ; namespace `plage_beach_cave_v1`), `exports/plage_beach_cave_v1/`, `apercu_plage_beach_cave_v1.html`. Moteur : PMDC v0.8.12 (`pmdc-linux-x64.zip`, SHA-256 c64f72af…, blob du dépôt RUNTIMEPMDO) + DumpAsset @3e767571 réinstallés dans `.cache/pmdo-runtime/` (tarball API, 339 Mo, ~3 min) ; `runtime_test.py` : `GetGround` natif PASS (45 × 20, TexSize 3, 3 calques, mer 17 frames / FrameLength 16 / feuille `PLAGE_BC1_ANIM`), hook retiré. Non fait et dit : rendu GPU/éditeur/gameplay, variante `dusk_beach` (flipbook plein cadre avec bulles cuites : toute insertion duplique les bulles), bulles/étincelles (objets EoSO, pas des cellules). Les vérifications de raccord comparent chaque nouveau voisinage au pire voisinage du même calque dans l'original (calque seul, RGBA) : ne pas mesurer sur le composite, un caillou sur du sable y vaut ≈ 6000 quel que soit le voisin.
