...
## Reprise maps V17 — Glace & Aurore canoniques sud-nord (25 septembre 2026)

Choix utilisateur : glace/aurore, methode mixte, livraison PNG to Tileset 8px, animations eau/aurore. Lot `source/glace_aurore_canonique_v1/`, `renders/glace_aurore_canonique_v1/`, viewer `apercu_glace_aurore_canonique_v1.html`, ZIPs pack + `GLACE_V17_png_import_8px.zip` (noms uniques a plat). Mixte = composition guidee par `layout_guide.png` sud-nord existant + 100% pixels natifs sur couches fixes, AUCUNE generation IA dans ce lot. 3 maps : BG aurore 264x216 (ciel comble par plus proche voisin natif, etoiles RGB natifs alpha seule 4f, aurore V10 a 1x 10f onde verticale pure boucle exacte, frise), arene 504x408 (mur nord remonte, crete sud ouverte, eboulis masques, fissures), route 504x360 (montagnes intactes, defile nord, blocs sud ouverts, lac pave continu y88-360 sans trou). Coupes organiques V/H suivant crevasses sombres (DP seam), sols paves natifs sans fondu, NPZ source_sxy + TSX 8px partout, chemins degages 0px (pixels, pas collision). Reflets 4f pulse A-B-A-C ±5% documente NOUVEAU. Lecons : distance_transform_edt prend les TROUS en entree (pas le plein) ; frame0 onde != extraction (phase t=0 non nulle, normal — tester cloture + verticalite, pas egalite f0) ; les 230 specks faible-alpha touchent tous un halo fort = franges legitimes, pas des etoiles ; np.dstack sur selections booleennes = piege de forme, preferer assignation par canal. 13 tests PASS, JS parse OK, 34 PNG viewer relus. Pas de navigateur interactif ni PMDO ; art non approuve. Poursuivre avec les autres zones pending du FULL_PROGRAMME (aride, jungle cascades, lac cristal...), sans toucher ce lot.
