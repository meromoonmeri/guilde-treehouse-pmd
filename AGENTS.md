...
### Entrée Amp V1 — « même endroit, autre lieu » (26 septembre)

Utilisateur : « faut vraiment que le lieu donne l'impression que c'est le même endroit mais un autre lieu ». Réf. `Amp_Plains_entrance_TD.png` (456×408, herbe olive atténuée, roche bleu-gris). Une première version sans guide (herbe jaune vif saturée) a été **écartée après comparaison colorimétrique** ; les bruts décor/sol/touffes retenus sont générés avec la ref en guide (`images=[ref]`). Fidélité mesurée : herbe décor (133,5, 138,6, 107,1) vs ref (138,5, 145,6, 110,8), distance 9,6 < 35 (`test_canonical_grass`, même masque des deux côtés). Le lot `amp_plains_fleurie_v1` existant est une variante verte fleurie, pas une entrée sud-nord : pas de doublon.

Pipeline : `source/entree_amp_sud_nord_v1/` (build autonome + utils bristle via `loadmod`), 9 calques + Top, 10 touffes (12×10, cycle sinusoïdal **pleine amplitude mesurée** — la version médiane de bristle n'utilisait que la moitié haute des poses), 7 étincelles (8 poses×2 + 8 repos = 24×5, palette propre 12 couleurs, érosion anti-frange 1 px avec repli). Arbres : les troncs (lum 60-120, r-b ±8) chevauchaient roche et plaine → détecteur dédié (lum<125 & r-b<8, hors rochers dilatés 12 px, compos ≥ 150). Sol normalisé par cover uniforme + recadrage (taille libre → 1200×896). Le `place()` partage un `taken` commun (pas de chevauchement touffes/étincelles, test d'égalité exacte du lifecycle sur l'émetteur 0). 9 tests PASS. Pas de runtime. Générations du lot : 8 (décor×3 avec 2 écartés, sol×2, touffes×2, étincelles×1).

### Entrée Horn V1 — montagne ocre, buissons par micro-relief (26 septembre)

Réf. `Mt_Horn_entrance_Sky.png` (552×360). Bruts décor/sol guidés par la ref ; sable décor (217,3, 162,2, 92,5) vs ref (189,2, 145,6, 80,4), distance 34,8 < 40 (`test_canonical_sand` sur couches cour+sentier). Tout est ocre : roche = germes sombres + croissance bornée 8 px ; sentier = lisse (std11<9) et clair touchant le bas. Buissons : les règles colorimétriques prenaient des plaques de sable clair (10 faux positifs) — remplacées par **densité de std3>12 en fenêtre 31 px > 0,40**, détectée AVANT la roche (sinon mangée par sa croissance) : exactement les 6 buissons, zéro extra. Anims : éboulis 1×8 et poussières 2×5 (lue en ligne) ; grilles magenta des planches gérées par règle de fond élargie + bornes par extremum local près des multiples du pas. Érosion anti-frange + palettes 12. Deux cycles 24×5, scène 120 ticks. 9 tests PASS. Pas de runtime. 5 générations (sol relancé1 fois, prompt court).
