...
- `sprite/` : **sprites Dynamax de tous les Pokémon animés de SpriteCollab** (978 espèces, forme de base) plus Falinks et Zarude de ce dépôt : pour chaque espèce, toutes les animations du sprite d'origine reprises (index, durées, `CopyOf`, déplacements d'ancre), dessin agrandi × 3 pixel par pixel, aura rouge animée et trois nuages tournants, `ShadowSize` 2 ; feuilles Anim / Offsets / Shadow au format SpriteCollab, `credits.txt`, `index.json`. Le **VFX de transformation** (`sprite/vfx/`) est un effet générique dessiné image par image (12 images, 1 s), sans personnage ni fond, en deux gabarits : rayon, colonne d'énergie opaque, éclairs épais qui tournent en s'abattant, flash, onde de choc. Hors du kit de la guilde : un dossier à part, ses sources dans `source/sprite/`. Voir [leur README](sprite/README.md).
