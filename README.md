...
- **Dix sprites officiels complétés — 22 animations de scène chacun** (Eat, Wake, Sit, LookUp, Sink, Nod, Pose, Pull, Pain, Float, DeepBreath, Trip, Laying, LeapForth, Head, Cringe, LostBalance, Tumble, TumbleBack, HitGround, Faint, EventSleep), composées de leurs propres cases officielles sur le squelette de Bayleef #0155, **sans repeindre un seul pixel**. Les gestes sont **physiologiques** et non de simples translations. Pour `Eat`, les **mains montent vers la bouche** — geste relevé au pixel près sur les `Eat` officiels de **Pichu #0172** et **Riolu #0447**, et rendu possible sans rien redessiner par les repères `lhand` / `rhand` / `head` que Chunsoft place dans chaque `-Offsets.png`. Pour `Nod`, `Sit`, `LookUp` ou `DeepBreath`, les appuis au sol restent fixes et seul le haut du corps plonge, se tend ou s'incline. Les amplitudes sont calculées sur la taille de chaque Pokémon :
  ...
  [Gardevoir #0282](personnages/gardevoir/animations_scenes/README.md) ·
...
- **`spritecollab/`** : **l'export au format exact du dépôt PMDCollab/SpriteCollab** — `sprite/<num>/` et `portrait/<num>/`, rien que les fichiers du format officiel (`AnimData.xml` en CRLF, feuilles Anim/Offsets/Shadow, `credits.txt`, portraits 40 × 40 et leurs miroirs `^`). C'est ce dossier qu'on déposerait sur le dépôt officiel. Voir [le rapport](spritecollab/RAPPORT.md).
- **Terapagos #1024, forme Terastal** (`1024/0001`) : les **22 animations de scène** ajoutées au sprite officiel, et une planche de **13 portraits** (6 officiels repris + 7 produits par générateur d'images et retenus au contrôle visuel). La forme de base #1024 reçoit également ses 22 animations. Voir `spritecollab/sprite/1024/0001/` et `spritecollab/portrait/1024/0001/`.
- **`Eat` produits par générateur d'images** pour Gardevoir #0282, Pancham #0674 et Slurpuff #0685 : le PNG du sprite officiel est soumis au générateur, puis sa sortie est ramenée sur la grille exacte, rabattue sur la palette du sprite (178-251 couleurs → 9-11) et limitée au rectangle de la bouche. 86 à 90 % du sprite conservé. Hariyama et Miltank, essayés de même, ont été **écartés** (68 % et 15 % : personnage méconnaissable) — voir `source/personnages/eat_generateur.py`.
- **`Eat` dessinés à la main** (bouche qui s'ouvre vraiment, peinte dans la palette du sprite à la manière des artistes Chunsoft) pour [Politoed #0186](personnages/politoed/eat_dessine/README.md), Ambipom #0424, Gible #0443, Pawmot #0923 et Dedenne #0702 — chacun sur son trait caractéristique, contrôlés par une grammaire graphique mesurée sur les `Eat` de Pichu #0172 et Riolu #0447.
- **Quatre planches de portraits complétées à 16 émotions** (40 × 40, ≤ 15 couleurs, versions `^`, planche SpriteBot 200 × 320), par retouche pixel du portrait Normal officiel :
  ...
  [Ambipom #0424](portraits/ambipom/README.md) ·
  [Pawmot #0923](portraits/pawmot/README.md).
