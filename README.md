...
- `personnages/zarude/` : **sprite de donjon de Zarude au format SpriteCollab** — découpe de la planche de marche fournie (4 directions, 11 couleurs) complétée par les diagonales et les poses d'animation, sur le squelette d'animation de Rillaboom #0812 : set donjon complet avec Sing (13 animations), 8 directions, `AnimData.xml`, feuilles Anim / Offsets / Shadow, Aseprite animé, variantes nuit, aperçus et vérificateur. Voir [leur README](personnages/zarude/README.md).
- `personnages/dynamax/` : **dix sprites Dynamax au format SpriteCollab** — Tarpaud, Écrémeuh, Gardevoir, Hariyama, Capidextre, Griknot, Pandespiègle, Pâtachiot (sprites SpriteCollab) plus Falinks et Zarude (ce dépôt) : toutes les animations du sprite d'origine reprises (index, durées, `CopyOf`, déplacements d'ancre), dessin agrandi × 3 pixel par pixel, aura rouge animée, `ShadowSize` 2 ; feuilles Anim / Offsets / Shadow, nuit, Aseprite, aperçus, vérificateur. Les **VFX Dynamax** (`personnages/dynamax/vfx/`) sont des effets génériques sans personnage ni fond, à superposer sur n'importe quel sprite : transformation (rayon, colonne d'énergie opaque, éclairs épais qui tournent en s'abattant, flash, onde de choc), apparition puis boucle des trois nuages tournants, aura ; deux tailles. Voir [leur README](personnages/dynamax/README.md).
