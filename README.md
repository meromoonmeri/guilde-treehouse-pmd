...
- `personnages/dynamax/` : **dix sprites Dynamax au format SpriteCollab** — Tarpaud, Écrémeuh, Gardevoir, Hariyama, Capidextre, Griknot, Pandespiègle, Pâtachiot (sprites SpriteCollab) plus Falinks et Zarude (ce dépôt) : toutes les animations du sprite d'origine reprises (index, durées, `CopyOf`, déplacements d'ancre), dessin agrandi × 2 pixel par pixel, aura rouge tramée et trois nuages tournants, `ShadowSize` 2 ; feuilles Anim / Offsets / Shadow, nuit, Aseprite, aperçus, vérificateur. Voir [leur README](personnages/dynamax/README.md).
