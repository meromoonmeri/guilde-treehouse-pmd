...
- **Crooked Cavern verdoyante v2 magenta (22/09/2026)** : mêmes zone et audit, mais **chaque calque généré séparément sur fond magenta** (chemin, parois, entrée, rochers, feuille de végétation, arbres) puis détouré et empilé ; jour + nuit, ORA → `renders/crooked_verdoyant_v2_magenta/README.md`, [galerie](apercu_crooked_verdoyant_v2_magenta.html).
- **Crooked Cavern verdoyante v2 — exports multicalques (22/09/2026)** : les 9 calques magenta en PNG + Aseprite + Tiled (atlas 8 px dédié), jour et nuit, relecture indépendante → `exports/crooked_verdoyant_v2_magenta/README.md`.
- **Crooked Cavern verdoyante v3 échelle (23/09/2026)** : mêmes bruts magenta, canevas **928×1152** sans downscale (arbres ~180 px / Vast Steppe 144×120, chemin 84–117 px, carte > caméra 640×360 et 848×480) → `renders/crooked_verdoyant_v3_echelle/README.md`, [galerie](apercu_crooked_verdoyant_v3_echelle.html), exports `exports/crooked_verdoyant_v3_echelle/`.
- **Crooked V3 fleurs Halcyon (23/09/2026)** : fleurs Vast Steppe natives 24×24, séquence 0,1,0,2, horloges 8/10/14, compositions PNG + WebP jour/nuit → `renders/crooked_verdoyant_v3_echelle/fleurs_halcyon/`, [galerie](apercu_crooked_v3_fleurs_halcyon.html).
- **Forêts lisière v1 (23/09/2026)** : 4 cartes 928×1152 (clairière grotte nord, forêt entrée ouest, caverne verdoyante, fond de forêt), 9 calques jour/nuit, exports multicalques → `renders/forets_lisiere_v1/`, [galerie](apercu_forets_lisiere_v1.html).
- **Forêts lisière v2 (23/09/2026)** : 4 cartes de plus (entrée est, couloir NE, caverne mousse, clairière secrète) 928×1152 multicalques → `renders/forets_lisiere_v2/`, [galerie](apercu_forets_lisiere_v2.html). v1 A–D inchangée.
- **Halcyon natif v1 (23/09/2026)** : 5 cartes en textures canoniques (2 layouts Steppe+Crooked+Relic, Altere Pond, Relic Forest, Vast Steppe entrance), multicalques → `renders/halcyon_natif_v1/`, [galerie](apercu_halcyon_natif_v1.html).
- **Sud–nord V4 (24/09/2026)** : entrée aride 408×560 + couloir violet 504×488 (programme des 23, 16 layouts restants), pixels natifs des références, PNG jour/nuit + paquet natif `.rsground`/`.tile` pixel-identique → `exports/zones_south_north_v4/`, [galerie](apercu_entrees_sud_nord_v4.html).
- **Entrée Crooked v1 (24/09/2026)** : méthode map ⇒ layers de génération (auditée, existe déjà), paroi/bouche/chemin générés style Crooked + herbe/fleurs Sky Peak, arbres Halcyon et rochers Crooked natifs, 8 calques jour/nuit, fleurs 4 phases → `renders/entree_crooked_v1/`, [galerie](apercu_entree_crooked_v1.html).
- **Entrée Crooked v2 (24/09/2026)** : les 8 calques générés (maquette = composite v1, refs canoniques Crooked/Halcyon/Sky Peak), recalés 1:1 (translations snap 8, bouche = référence), jour/nuit Abyss, ORA → `renders/entree_crooked_v2/`, [galerie](apercu_entree_crooked_v2.html). Fleurs statiques (pack v1 animé conservé).
- **Entrée Crooked FINALE (24/09/2026)** : map finale générée d'un bloc (guide V1 + refs canoniques Halcyon/EoSO/DumpAsset) puis **décomposée en 8 calques** (partition exacte, composite == brut diff 0), 65 fleurs, 4 arbres, 6 rochers, sol complet reconstitué, nuit Abyss exacte, 17 PASS → `renders/entree_crooked_finale/`, [galerie](apercu_entree_crooked_finale.html).
