...
`entrancecascade.png` (vraie entrée de Waterfall Cave) sert désormais à EWC1. `Mystifying_Forest_entrance_TDS.png` sert désormais à EMF1. `Dark_Crater_Pit_TDS.png` : EDP1 et son doublon ECF1 (cellules de Voronoï, `lava_frames`). `Underground_Lake_shore_TDS.png` sert désormais à EUL1 (lac souterrain, chaussée jusqu'à la grotte). `Foggy_Forest_Base_Camp_TDS.png` sert désormais à EFF1 (camp de base, tentes, brume tramée en damier alpha 0/255). TMA1 (`source/thunder_meadow_arene_v1/`) = Thunder Meadow (« tiny meadow ») : falaise à terrasses + arène + îlots, fond spirale d'orage re-indexé sur la palette de flash canonique de `5394.png` (6 niveaux natifs), foudre générée, étincelles natives ; prairie/falaises/pierres projetées sur les couleurs exactes du rip (rip et planche repris de la branche arena/01a0d8a8). `Southern_Jungle_exit_S.png` sert désormais à ESJ1 (`source/entree_southern_jungle_sud_nord_v1/`, sortie de jungle, mare Métano, papillons générés ; décor re-teinté une fois vers le rip, fidélité sable 8.9 / herbe 10.4). ECC2 (`source/entree_crystal_cave_groudon_v2/`) = ECC1 + VFX colonnes de flamme pour l'arrivée de Groudon : 3 calques d'événement invisibles (48×4 ticks, joués une fois), `init.lua arrivee_legendaire()` NON TESTÉ. EFF2 = même carte sans tentes (herbe recopiée du même décor), arbres en 3 calques ombres/troncs/houppiers (`split_trees`). `Dark_Crater_Pit_TDS.png` sert désormais à EDP1 (fosse de lave : 4e brut `lave_complete` par édition du décor, lave ramenée aux 11 couleurs du rip et animée en cycle de palette). Références jamais utilisées comme référence principale de la série, toutes branches confondues : GBA Mushroom Forest et Mt. Thunder (DA GBA), `Sealed_Ruin_pit_TDS.png`, `Southern_Jungle_exit_S.png`, `Southern_Jungle_exit_2_S.png` et `Waterfall_Cave_ledge_TDS.png`. Revérifier les branches sœurs avant de choisir.
`entrancecascade.png` (vraie entrée de Waterfall Cave) sert désormais à EWC1. `Mystifying_Forest_entrance_TDS.png` sert désormais à EMF1. `Underground_Lake_shore_TDS.png` sert désormais à EUL1 (lac souterrain, chaussée jusqu'à la grotte). `Waterfall_Cave_gem_TDS.png` sert désormais à ECC1 (grotte de cristal, éclats sur cristaux, eau aux couleurs du rip via `UL.WPAL`). `Foggy_Forest_Base_Camp_TDS.png` sert désormais à EFF1 (camp de base, tentes, brume tramée en damier alpha 0/255). EFF2 = même carte sans tentes (herbe recopiée du même décor), arbres en 3 calques ombres/troncs/houppiers (`split_trees`). Références jamais utilisées comme référence principale de la série, toutes branches confondues : GBA Mushroom Forest et Mt. Thunder (DA GBA), `Sealed_Ruin_pit_TDS.png`, `Southern_Jungle_exit_2_S.png` et `Waterfall_Cave_ledge_TDS.png`. Revérifier les branches sœurs avant de choisir.
