...
**Clarification utilisateur du 26 septembre : appliquer la méthode aux nouvelles maps seulement ; ne pas reprendre les cartes déjà livrées.** Choisir une référence encore disponible dans le dépôt si l'utilisateur délègue ce choix. Utiliser le même pipeline que les lots récents : composition complète générée sur fond magenta, sol séparé, calques alignés, exports PNG 8 px et Ground PMDO, sans mosaïque de fragments de maps. Important : terrain généré en DA PMD ≠ pixels natifs canoniques certifiés ; distinguer chaque source et ne pas revendiquer une fidélité pixel-exacte. Si le sens de « texture canonique », la référence ou la portée d'un lot est ambigu, demander avant de construire.

**Nouvelle carte pilote de cette reprise :** Entrée Grotte des Cascades (`source/entree_grotte_cascade_sud_nord_v1/`, `renders/entree_grotte_cascade_sud_nord_v1/`), référence `Waterfall_Cave_ledge_TDS.png`; 768 × 576, terrain/sol/eau générés, 12 phases d'eau × 10 ticks, deux marqueurs sans warp. Les six anciens lots restent inchangés.
