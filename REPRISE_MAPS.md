...
`entrancecascade.png` (vraie entrée de Waterfall Cave) sert désormais à EWC1. `Mystifying_Forest_entrance_TDS.png` sert désormais à EMF1. Références jamais utilisées comme référence principale de la série, toutes branches confondues : `Underground_Lake_shore_TDS.png` (rive de sable, lac luminescent, parois de grotte), `Foggy_Forest_Base_Camp_TDS.png` (campement), GBA Mushroom Forest et Mt. Thunder (DA GBA), `Sealed_Ruin_pit_TDS.png` et `Southern_Jungle_exit_2_S.png` (salles de fond). Revérifier les branches sœurs avant de choisir.
