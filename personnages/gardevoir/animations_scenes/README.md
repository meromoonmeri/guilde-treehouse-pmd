# Gardevoir #0282 (Gardevoir) — animations de scène ajoutées

![Aperçu](apercu.png)

Le sprite officiel de Gardevoir sur [PMDCollab](https://sprites.pmdcollab.org/#/0282?form=0) a le set de donjon, mais
...
Chaque image part d'une **case officielle de Gardevoir lui-même** (Idle, Hurt, Hop, Charge, Rotate,
Sleep…). La palette des nouvelles feuilles est incluse dans celle du sprite d'origine
(10 couleurs) — le vérificateur le refuse sinon.
...
`-Offsets.png` marque `head`, `lhand` et `rhand` sur chaque case. Pour Gardevoir, les repères
...
donc calculées sur la hauteur réelle de la silhouette au repos de **Gardevoir** (23 px) :
...
| `gardevoir_scenes.aseprite` | Aseprite animé : un calque par direction, les 22 animations bout à bout, une étiquette chacune. |
...
| `Tumble` | 16 | 32 × 48 | 8 | 1 | 24 ticks | Rotate |
| `Pose` | 17 | 32 × 48 | 3 | 8 | 22 ticks | Charge, Idle |
| `Pull` | 18 | 32 × 40 | 7 | 1 | 76 ticks | Charge |
...
| `LeapForth` | 28 | 32 × 112 | 6 | 1 | 14 ticks | Hop |
| `Head` | 29 | 32 × 56 | 1 | 8 | 4 ticks | Charge |
| `Cringe` | 30 | 24 × 64 | 2 | 1 | 10 ticks | Hurt |
| `LostBalance` | 31 | 32 × 56 | 2 | 1 | 16 ticks | Hurt |
| `TumbleBack` | 32 | 32 × 48 | 10 | 1 | 30 ticks | Rotate |
| `HitGround` | 34 | 24 × 56 | 8 | 1 | 34 ticks | Hurt |
| `Faint` | 33 | 48 × 56 | 4 | 8 | 34 ticks | Hurt |

Animations d'origine conservées telles quelles : `Walk`, `Attack`, `Strike (copie de Attack)`, `Shoot (copie de Charge)`, `SpAttack (copie de Appeal)`, `Appeal`, `Sleep`, `Hurt`, `Idle`, `Swing`, `Double`, `Hop`, `Charge`, `Rotate`.
