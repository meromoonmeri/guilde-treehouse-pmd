# Pawmot #0923 (Pawmot) — animations de scène ajoutées

![Aperçu](apercu.png)

Le sprite officiel de Pawmot sur [PMDCollab](https://sprites.pmdcollab.org/#/0923?form=0) a le set de donjon, mais
...
Chaque image part d'une **case officielle de Pawmot lui-même** (Idle, Hurt, Hop, Charge, Rotate,
...
`-Offsets.png` marque `head`, `lhand` et `rhand` sur chaque case. Pour Pawmot, les repères
...
donc calculées sur la hauteur réelle de la silhouette au repos de **Pawmot** (26 px) :
...
| `pawmot_scenes.aseprite` | Aseprite animé : un calque par direction, les 22 animations bout à bout, une étiquette chacune. |
...
| `Laying` | 27 | 24 × 48 | 1 | 8 | 12 ticks | Sleep |
...
| `Faint` | 33 | 48 × 64 | 4 | 8 | 34 ticks | Hurt |

Animations d'origine conservées telles quelles : `Walk`, `Attack`, `QuickStrike`, `Shoot`, `Shock`, `Sleep`, `Hurt`, `Idle`, `Swing`, `Double`, `Hop`, `Charge`, `Rotate`.
