...
R=Path(__file__).resolve().parents[2];REF=R/'source/antre_harmonie_v3/references';O=R/'renders/antre_raccords_v5';P=O/'antre';S=O/'sprites';P.mkdir(parents=True,exist_ok=True);S.mkdir(exist_ok=True)
...
# Bank layers live exclusively on the wet side and match all irregular rock contours.
# Keep the native platform water cycle untouched: new bank effects are only around cavern rock.
bankdist=nd.distance_transform_edt(~rock);bank=(bankdist>0)&(bankdist<=3)&~dry
shorephases=[]
for p in range(4):
 a=np.zeros((H,W,4),dtype='uint8');near=bank&(bankdist<=1.45);outer=bank&~near
 a[:,:,:3]=[72,158,181];a[:,:,3]=near*32
 # Very light discontinuous water-edge highlights, phase shifts of1px, not a bright uniform outline.
 pulse=((xx+2*yy+p)%7)<3;a[outer,:3]=[186,233,237];a[:,:,3][outer]=np.where(pulse[outer],23,12);a[a[:,:,3]==0]=0
...
# Fixed layout-aware horizontal warps for native foam: compress into the same connected
# water pocket instead of cutting the white bank against pillars frame by frame.
foamwarps=[]
for f in falls:
 mappings=[]
 for row in range(28):
  y=f['foot_y']-8+row;line=~dry[y];labs,n=nd.label(line);candidates=np.flatnonzero(line & (abs(np.arange(W)-f['x'])<=32))
  if len(candidates)==0:mappings.append(None);continue
  anchor=int(candidates[np.argmin(abs(candidates-f['x']))]);run=np.flatnonzero(labs==labs[anchor]);lo=max(int(run[0]),f['x']-32);hi=min(int(run[-1])+1,f['x']+33)
  width=min(48,max(0,hi-lo-2))
  if width<4:mappings.append(None);continue
  left=int(np.clip(f['x']-width//2,lo+1,hi-width-1));mappings.append((left,width))
 foamwarps.append(mappings)
for phase in range(3):
 ls=[]
 for k,f in enumerate(falls):
  sp=np.array(foams[phase].resize((48,28),NN));a=np.zeros((H,W,4),dtype='uint8')
  for row,mapping in enumerate(foamwarps[k]):
   if mapping is None:continue
   left,width=mapping;y=f['foot_y']-8+row;cols=np.minimum(47,np.floor(np.arange(width)*48/width).astype(int));a[y,left:left+width]=sp[row,cols]
  # Persistent aerated contact core joins all4chute poses to all3foam poses.
  throat=(abs(xx-f['x'])<=max(3,f['width']//3))&(yy>=f['foot_y']-2)&(yy<=f['foot_y']+3)&~dry
  a[throat]=[239,248,255,245];a[dry]=0
  # No remote detached white flecks: retain components touching the impact core.
  labels,count=nd.label(a[:,:,3]>0,np.ones((3,3)));keep=np.unique(labels[throat]);keep=keep[keep>0];a[~np.isin(labels,keep)]=0
...
for n,im in static:im.save(P/f'raccord5_{n}.png')
frames=[];dur=[round((p+1)*1000/6)-round(p*1000/6) for p in range(N)]
for p in range(N):
 for n,im in animated(p):im.save(P/f'raccord5_{n}_{p:02}.png')
 im=compose(layers(p));assert np.all(np.array(im)[:,:,3]==255);im.save(P/f'raccord5_composition_{p:02}.png');frames.append(im)
...
with zipfile.ZipFile(P/'antre_raccords_v5.ora','w',zipfile.ZIP_DEFLATED) as z:
...
m={'size':[W,H],'frames':N,'durations_ms':dur,'loop_ms':4000,'game_frames_per_pose':10,'cycles':{'basin_generated':4,'cascades_regenerated':4,'foam_native_adapted':3,'platform_water_native':8},'static':[n for n,_ in static],'animated':[n for n,_ in animated(0)],'order':[n for n,_ in layers(0)],'falls':falls,'platform':{'origin':list(origin),'source_rects':[[112*(p%4),984+144*(p//4),112*(p%4)+112,1128+144*(p//4)] for p in range(8)],'disc_native_bbox':[20,9,92,62],'disc_size':[72,53],'previous_disc_size':[160,106],'stone_pixels_unchanged':True,'scale':1},'native_platform_proof':proof,'native_atlas_sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'material':'V4basin poses attenuated to12percent contrast on nativeAltere water base; rock-side wet bank3px; V4falls preserved; native3key foam layout-warped with persistent contact core;8key platform water unchanged','bank_width_px':3,'bank_max_alpha':32,'basin_delta_limit_rgb':[-12,10],'foam_layout_warp':True,'foam_contact_core':True,'runtime_pmdo_validated':False}
...
for n,_ in animated(0):
 ims=[load(P/f'raccord5_{n}_{p:02}.png') for p in range(N)];b=io.BytesIO();ims[0].save(b,format='WEBP',save_all=True,append_images=ims[1:],duration=dur,lossless=True,loop=0);items.append({'name':n,'src':uri(b.getvalue(),'image/webp')})
html='''<!doctype html><html lang="fr"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Antre · raccords et eau subtile V5</title><style>body{background:#292c29;color:#f1eddc;max-width:1000px;margin:30px auto;padding:0 20px;font:16px system-ui}p{line-height:1.6}img{image-rendering:pixelated;width:960px;max-width:100%;background:repeating-conic-gradient(#3a4643 0 25%,#4c5952 0 50%) 0/16px 16px}select{padding:10px;margin:10px;background:#4b5549;color:white}</style><h1>Antre · détails et raccords · V5</h1><p>Disque réduit à sa taille native72×53px, pierres immobiles, cycle natif complet de8poses pour les rides/reflets. Bassin atténué sur le bleu natif de Métano/Altere ; rives animées suivant les roches ; écumes adaptées aux poches d’eau au pied des chutes, sans morceaux blancs détachés. Boucle commune4secondes, calques indépendants.</p><select id="sel"></select><br><img id="view" alt="Antre au bassin dégagé"><p>Les parois Crooked de la V3 sont conservées. Les cascades régénérées sont des adaptations, pas des sprites officiels. Les pierres et leurs rides sont des pixels natifs à échelle1. Aucun test PMDO.</p><script>const items='''+json.dumps(items)+''';items.forEach((x,i)=>sel.add(new Option(x.name,i)));view.src=items[0].src;sel.onchange=()=>view.src=items[+sel.value].src;</script>'''
(R/'apercu_antre_raccords_v5.html').write_text(html)
