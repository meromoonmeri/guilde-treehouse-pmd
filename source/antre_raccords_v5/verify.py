...
R=Path(__file__).resolve().parents[2];O=R/'renders/antre_raccords_v5';P=O/'antre';S=O/'sprites';m=json.loads((O/'manifest.json').read_text());W,H=m['size']
...
def merge(layers):
 ...
 return out
static={n:im(P/f'raccord5_{n}.png') for n in m['static']};rocknames=['02_paroi_fond','03_bordure_gauche','04_bordure_droite','05_rebord_bas'];platformnames=['06_disque_natif']+[f'07_pas_japonais_{k}' for k in range(1,4)]
...
for p in range(m['frames']):
 layers=dict(static)
 for n in m['animated']:
  layers[n]=im(P/f'raccord5_{n}_{p:02}.png');arr=np.array(layers[n]);assert layers[n].size==(W,H)
  if n!='00_bassin_regenere':assert not np.any(arr[:,:,3][dry]),(p,n)
  period=8 if n.startswith('08_') else 3 if n.startswith('20_') else 4
  assert np.array_equal(arr,a(P/f'raccord5_{n}_{p%period:02}.png'))
 out=np.array(merge([layers[n] for n in m['order']]));assert np.all(out[:,:,3]==255);assert np.array_equal(out,a(P/f'raccord5_composition_{p:02}.png'));assert np.array_equal(out[stone[:,:,3]>0],stone[stone[:,:,3]>0])
...
with zipfile.ZipFile(P/'antre_raccords_v5.ora') as z:
...
wet=~dry;distance=nd.distance_transform_edt(~rock);edge=(distance>0)&(distance<=3)&wet
new=[];old=[]
for p in range(4):
 new.append(a(P/f'raccord5_00_bassin_regenere_{p:02}.png')[:,:,:3].astype(float))
 old.append(a(R/f'renders/antre_bassin_v4/antre/bassin4_00_bassin_regenere_{p:02}.png')[:,:,:3].astype(float))
 bank=a(P/f'raccord5_09_rives_roche_eau_{p:02}.png');bm=bank[:,:,3]>0
 assert not np.any(bm&~edge);assert bank[:,:,3].max()<=32
 assert np.all(new[-1][edge]==[131,218,230])
...
for p in range(24):
 for k in range(1,7):
  fa=a(P/f'raccord5_20_ecume_{k}_{p:02}.png')[:,:,3]>0;stream=a(P/f'raccord5_10_cascade_{k}_{p:02}.png')[:,:,3]>0;labs,count=nd.label(fa,np.ones((3,3)))
  assert count>0
  for lab in range(1,count+1):assert np.any((labs==lab)&stream)
print('PASS additional:bank only on wet3px border,stable basin tone at shores,all foam components attached to their cascade. Mean cyclic RGB change',new_motion,'versus V4',old_motion)
