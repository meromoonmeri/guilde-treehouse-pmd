...
R=Path(__file__).resolve().parents[2];O=R/'renders/arene_aquatique_bois_v1';P=O/'arene';m=json.loads((O/'manifest.json').read_text())
...
assert len(m['falls'])==6 and len(m['order'])==25
...
print('PASS:25 layers,40 exact opaque compositions,6 localized foam impacts, dry platform unchanged at every phase, exact ORA and40frameWebP. No engine validation.')
