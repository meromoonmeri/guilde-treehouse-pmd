...
const R=path.resolve(__dirname,'../..'),O=path.join(R,'renders/arene_glace_sky_peak_v3'),html=fs.readFileSync(path.join(O,'index.html'),'utf8');
...
(async()=>{
 ...
 check('Ordre : onde derrière lune et montagnes',draws.indexOf(data.aurora.frames[0])<draws.indexOf(data.layers['03_lune_canonique'])&&draws.indexOf(data.layers['03_lune_canonique'])<draws.indexOf(data.layers['04_montagnes_horizon']));
 ids.before.listeners.click();check('Sans onde/forêt : huit plans',draws.length===8&&!draws.some(p=>p.includes('sapins')||p.includes('_onde_')));
 ids.all.listeners.click();check('Tous les plans : dix',draws.length===10);
