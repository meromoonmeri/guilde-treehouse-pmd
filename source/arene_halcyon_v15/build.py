"""V15 : zone generee par la methode canonique (terrain plein cadre + magenta) et
convertie aux criteres Halcyon/Palika : calques fixes empiles, frames d'aurore a taille
identique et nommage uniforme (AuroreV15_00..07.png), duree uniforme, position sur
...
O = R / 'renders/arene_halcyon_v15'
BRUT_T = O / 'bruts/terrain_complet.png'
BRUT_B = O / 'bruts/boreale_ondulations_8.png'
...
def construire():
    ...
    for f, im in enumerate(frames):
        im.save(O / 'couches/aurore' / f'AuroreV15_{f:02d}.png')
    frames[0].save(O / 'aurore_ondulation_8frames.webp', save_all=True, append_images=frames[1:], duration=MS, loop=0, lossless=True, method=4)
    # --- scenes ---
    def scene(f):
    ...
    (R / 'apercu_arene_halcyon_v15.html').write_text(
    ...
    (O / 'manifest.json').write_text(json.dumps({
        'halcyon': {'grille_px': 8, 'aurore_position': [POS_X, POS_Y], 'position_multiple_de_8': POS_X % 8 == 0 and POS_Y % 8 == 0,
                    'frames_nommage_uniforme': 'AuroreV15_00..07.png', 'frames_taille_uniforme': [CASE_L, CASE_H],
                    'duree_ms_uniforme': MS, 'ordre_des_calques': ['ciel_fixe', 'etoiles_fixes', 'aurore (8 frames)', 'terrain_fixe'],
                    'pas_de_wrap': True},
