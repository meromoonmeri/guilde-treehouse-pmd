"""Build Beach Cave Entrance V3 from canonical EoSO cells only.
...
OUT = ROOT / "renders" / "beach_cave_entrance_v3"
PACK = ROOT / ".cache" / "beach_cave_entrance_v3_pack"
ZIP = ROOT / "beach_cave_entrance_v3_pmdo.zip"
W, H, CELL = 40, 20, 24
GROUND_NAME = "v50812_beach_cave_entrance_v3.rsground"
...
def build_layers() -> tuple[list[list[dict]], dict]:
    ...
    back_w = len(source_back["Tiles"])
    anim_w = len(source_anim["Tiles"])

    # Wide open beach: extend only by repeating complete canonical cells. The
    # sky/shore band comes from the audited Beach reference; the large sand
    # arrival uses one of its native sand cells at 24 px, never a repaint.
    ...
    for x in range(W):
        for y in range(H):
            if y < 6:
                back[x][y] = copy.deepcopy(source_back["Tiles"][x % back_w][y])
            elif y == 6:
                back[x][y] = copy.deepcopy(shore)
            else:
                back[x][y] = copy.deepcopy(sand)

    # Preserve the complete native upper animation and FrameLength=16. The
    # 40-cell width is extended with canonical source columns only.
    anim = [[blank() for _ in range(H)] for _ in range(W)]
    for x in range(W):
        for y in range(7):
            anim[x][y] = copy.deepcopy(source_anim["Tiles"][x % anim_w][y])

    # East lagoon: a broad native water band assembled from the water-facing
    # cells of BeachCavePit. Repeated cells remain exact canonical payloads;
    # no interpolation, recoloring or generated water is introduced.
    water = [[blank() for _ in range(H)] for _ in range(W)]
    water_band = [7, 8, 9, 10, 11, 12, 11, 12]
    dx0, dy0 = 25, 10
    for dx, sx in enumerate(water_band):
        for dy, sy in enumerate((3, 4)):
            source_cell = source_pit["Tiles"][sx][sy]
            if source_cell["Layers"]:
                water[dx0 + dx][dy0 + dy] = copy.deepcopy(source_cell)

    # Foreground terraces leave a broad central promenade. The side banks,
    # palms and cave mouth are all moved from contiguous native cells; the
    # generated guide is not consulted for pixels.
    front = [[blank() for _ in range(H)] for _ in range(W)]
    for sx in range(0, 8):
        for sy in range(3, 16):
            front[sx][sy + 4] = copy.deepcopy(source_front["Tiles"][sx][sy])
    for sx in range(26, 33):
        for sy in range(3, 16):
            front[33 + (sx - 26)][sy + 4] = copy.deepcopy(source_front["Tiles"][sx][sy])

    # Palm terrace at west, using the same native group that is visible in the
    # EoSO Beach map but relocating its cells in the new wide layout.
    for sx in range(13, 19):
        for sy in range(9, 16):
            cell = source_front["Tiles"][sx][sy]
            if cell["Layers"]:
                front[sx - 9][sy] = copy.deepcopy(cell)

    # Native rock shoulders frame the east lagoon without covering its water.
    for sx in range(22, 25):
        for sy in range(9, 13):
            cell = source_front["Tiles"][sx][sy]
            if cell["Layers"]:
                front[sx + 1][sy + 1] = copy.deepcopy(cell)

    # A canonical cliff surround makes the cave mouth read as an entrance in
    # the wide sand field rather than as a floating isolated tile block.
    for sx in range(0, 4):
        for sy in range(3, 8):
            cell = source_front["Tiles"][sx][sy]
            if cell["Layers"]:
                front[14 + sx][sy + 4] = copy.deepcopy(cell)
                front[22 + sx][sy + 4] = copy.deepcopy(cell)

    # Friendly central cave mouth, assembled from the native Beach Cave Pit
    # arch cells and placed above the south-facing promenade.
    for sx in range(8, 12):
        for sy in range(0, 3):
            cell = source_pit["Tiles"][sx][sy]
            if cell["Layers"]:
                front[sx + 10][sy + 6] = copy.deepcopy(cell)
    ...
    spec = {
        "id": "beach_cave_entrance_v3",
        "title": "Beach Cave Entrance V3 — la Plage Douillette",
        "grid_cells": [W, H],
        "size_px": [W * CELL, H * CELL],
        "tex_size": 3,
        "tile_size_px": CELL,
        "layout": "wide cozy composition: central cave mouth, open south-north promenade, east lagoon, west palm terrace",
        ...
        "entrance": {"name": "Entrance", "xy": [472, 436], "direction": 4},
        "dungeon_threshold": {"name": "donjon_seuil", "xy": [472, 148], "direction": 0, "width_px": 48},
...
<h1>{spec["title"]}</h1><p>Layout différent de la référence : promenade sud–nord large, bouche centrale, lagune canonique à l'est et terrasse de palmiers à l'ouest. L'eau est animée avec les frames natives de <code>BeachCavePit.tile</code>; le bandeau haut conserve la liste native de <code>beach_animation.tile</code>.</p>
<button id="play">Pause</button> <input id="frame" type="range" min="0" max="{len(frame_uris)-1}" value="0"><span id="label">Phase 1</span><img id="scene" alt="Beach Cave Entrance V3">
...
<script>const frames={json.dumps(frame_uris)},scene=document.querySelector('#scene'),range=document.querySelector('#frame'),label=document.querySelector('#label'),play=document.querySelector('#play');let running=true,t=0;function draw(){{scene.src=frames[t];range.value=t;label.textContent='Phase '+(t+1)+' / '+frames.length}}range.oninput=()=>{{running=false;play.textContent='Lecture';t=+range.value;draw()}};play.onclick=()=>{{running=!running;play.textContent=running?'Pause':'Lecture'}};setInterval(()=>{{if(running){{t=(t+1)%frames.length;draw()}}}},130);draw();</script></main></html>'''
    (ROOT / "apercu_beach_cave_entrance_v3.html").write_text(html, encoding="utf-8")
...
def ground_document(spec: dict, layers: list[list[list[dict]]]) -> dict:
    ...
    obj.update({
        ...
        "Name": spec["title"],
        "Released": False,
        "AssetName": "v50812_beach_cave_entrance_v3",
        ...
        "Entities": [{"Name": spec["title"], "Visible": True, "MapChars": [], "GroundObjects": [], "Spawners": [], "Markers": [
            {"EntName": "Entrance", "Direction": spec["entrance"]["direction"], "EntEnabled": True, "EntOrder": 0, "InteractOrder": 0, "triggerType": 0, "Collider": {"X": spec["entrance"]["xy"][0], "Y": spec["entrance"]["xy"][1], "Width": 16, "Height": 16}},
            {"EntName": "donjon_seuil", "Direction": spec["dungeon_threshold"]["direction"], "EntEnabled": True, "EntOrder": 0, "InteractOrder": 0, "triggerType": 0, "Collider": {"X": spec["dungeon_threshold"]["xy"][0], "Y": spec["dungeon_threshold"]["xy"][1], "Width": 16, "Height": 16}},
        ]}],
    })
...
def package(ground_path: Path, manifest: dict) -> None:
    ...
    root = PACK / "beach_cave_entrance_v3"
    for p in [root / "Data/Ground", root / "Content/Tile", root / "Data/Script/ground/v50812_beach_cave_entrance_v3", root / "Data/Script/beach_cave_entrance_v3/ground/v50812_beach_cave_entrance_v3"]: p.mkdir(parents=True, exist_ok=True)
    ...
    for p in [root/"Data/Script/ground/v50812_beach_cave_entrance_v3/init.lua", root/"Data/Script/beach_cave_entrance_v3/ground/v50812_beach_cave_entrance_v3/init.lua"]:p.write_bytes(script)
    tile_paths=[]
...
def main():
    ...
    manifest={**spec,"ground":GROUND_NAME,"preview_frames":36,"runtime_engine_tested":False,"destination_configured":False,"guide":"renders/beach_cave_entrance_v3/guide_layout_cozy_magenta.png"}
