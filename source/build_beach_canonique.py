...
LAYOUTS = [
    {'id': '01_cote_reference', 'name': 'La côte de la référence',
     'description': 'Reprise fidèle de la référence : mer au nord, falaises cadrantes, pente de sable à gauche, deux bosquets de palmiers au milieu de la plage, rochers épars, rangée de falaises sur l’herbe au sud.',
     'shore': 6, 'blocks': [('falaise_gauche', 0, 3), ('falaise_droite', 35, 3),
                            ('pente_sable', 0, 11), ('palmiers_gauche', 10, 15),
                            ('palmiers_milieu', 20, 16), ('rochers_petits', 17, 11),
                            ('buisson', 24, 12), ('rochers_petits', 30, 14),
                            ('falaise_basse', 6, 24), ('palmiers_milieu', 15, 24),
                            ('falaise_basse', 26, 24), ('herbe_bas', 0, 28), ('herbe_bas', 33, 28)]},
    {'id': '02_maree_haute', 'name': 'La marée haute',
     'description': 'Même structure que la référence avec une mer plus présente, plage réduite et rivage plus haut.',
     'shore': 4, 'blocks': [('falaise_gauche', 0, 1), ('falaise_droite', 35, 1),
                            ('pente_sable', 0, 9), ('palmiers_gauche', 12, 14),
                            ('rochers_petits', 22, 10), ('rochers_petits', 28, 12),
                            ('buisson', 18, 12),
                            ('falaise_basse', 6, 24), ('palmiers_milieu', 15, 24),
                            ('falaise_basse', 26, 24), ('herbe_bas', 0, 28), ('herbe_bas', 33, 28)]},
    {'id': '03_ilot_au_large', 'name': 'L’îlot au large',
     'description': 'La référence élargie : un îlot rocheux natif face à la plage, palmiers et rochers comme dans la scène source.',
     'shore': 8, 'blocks': [('falaise_droite', 14, 1), ('falaise_gauche', 0, 5),
                            ('falaise_droite', 35, 5), ('pente_sable', 0, 13),
                            ('palmiers_gauche', 12, 18), ('palmiers_milieu', 22, 19),
                            ('rochers_petits', 18, 14), ('buisson', 28, 16),
                            ('falaise_basse', 6, 24), ('palmiers_milieu', 15, 24),
                            ('falaise_basse', 26, 24), ('herbe_bas', 0, 28), ('herbe_bas', 33, 28)]},
    {'id': '04_plage_abritee', 'name': 'La plage abritée',
     'description': 'Falaises empilées des deux côtés pour une crique plus fermée, palmiers centrés.',
     'shore': 6, 'blocks': [('falaise_gauche', 0, 3), ('falaise_gauche', 0, 11),
                            ('falaise_droite', 35, 3), ('falaise_basse', 35, 11),
                            ('palmiers_milieu', 16, 18), ('buisson', 12, 16), ('buisson', 26, 16),
                            ('rochers_petits', 22, 20),
                            ('falaise_basse', 6, 24), ('palmiers_gauche', 15, 24),
                            ('falaise_basse', 26, 24), ('herbe_bas', 0, 28), ('herbe_bas', 33, 28)]},
    {'id': '05_double_rivage', 'name': 'Le double rivage',
     'description': 'La référence coupée d’un chenal d’eau animée : mer au nord, chenal au sud, palmiers entre les deux.',
     'shore': 5, 'channel': 17, 'blocks': [('falaise_gauche', 0, 2), ('falaise_droite', 35, 2),
                                           ('palmiers_milieu', 16, 12), ('buisson', 12, 13),
                                           ('rochers_petits', 26, 13),
                                           ('falaise_basse', 6, 24), ('palmiers_milieu', 15, 24),
                                           ('falaise_basse', 26, 24), ('herbe_bas', 0, 28), ('herbe_bas', 33, 28)]},
    {'id': '06_lagon_clair', 'name': 'Le lagon clair',
     'description': 'Mer haute piquetée de deux îlots, grande plage basse et double bosquet de palmiers.',
     'shore': 10, 'blocks': [('falaise_droite', 12, 2), ('falaise_droite', 26, 4),
                             ('falaise_gauche', 0, 7), ('falaise_droite', 35, 7),
                             ('palmiers_gauche', 14, 22), ('palmiers_milieu', 22, 22),
                             ('rochers_petits', 20, 18), ('buisson', 30, 20),
                             ('falaise_basse', 6, 24), ('falaise_basse', 30, 24),
...
def build(cfg):
    ...
    # Calque avant : blocs natifs de layer2 (aucun chevauchement autorisé :
    # chaque cellule du calque doit rester une copie exacte d'une tuile native).
    used = set()
    for name, bx, by in cfg['blocks']:
        ...
        for ty in range(y0, y1 + 1):
            for tx in range(x0, x1 + 1):
                ...
                if t and t.getbbox() and 0 <= dx < GW and 0 <= dy < GH:
                    assert (dx, dy) not in used, f'{cfg["id"]}: blocs qui se chevauchent en ({dx},{dy})'
                    used.add((dx, dy))
    for name, bx, by in cfg['blocks']:
        sheet, x0, y0, x1, y1 = BLOCKS[name]
        for ty in range(y0, y1 + 1):
