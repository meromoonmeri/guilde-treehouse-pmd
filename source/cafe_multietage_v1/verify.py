...
R=Path(__file__).resolve().parents[2];P=Path(__file__).resolve().parent/'references';O=R/'renders/cafe_multietage_v1';m=json.loads((O/'manifest.json').read_text());checks=[]
...
for e in m['levels']:
 ...
 for mode in ['jour','nuit']:
  ...
  for n in ['01_sol_vide','02_vues','03_bois_murs','04_bordure_rocheuse','05_cadres_fenetres','06_escaliers']:comp.alpha_composite(view if n=='02_vues' else load(p/(n+'.png')))
...
checks+=['20 compositions exactement recomposees depuis six calques','10 toiles sur grille8px','Toutes vues exterieures strictement dans les ouvertures circulaires','Etage+2 : deux hublots32px espaces, aucune baie ; sous-sols sans fenetre']
for e in m['levels']:
 for link in e['connections']:
  target=next(t for t in m['levels'] if t['variant']==e['variant'] and t['floor_id']==link['target']);assert any(l['target']==e['floor_id'] for l in target['connections'])
checks.append('Liaisons de schema reciproques entre les cinq niveaux, pour A et B')
