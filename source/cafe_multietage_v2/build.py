...
R=Path(__file__).resolve().parents[2];P=R/'source/cafe_multietage_v1/references';O=R/'renders/cafe_multietage_v2';SOURCES={}
...
def extend(im,extra_x,extra_y,cx,cy):
 # Insert symmetrically OUTSIDE the original doorway: its width stays intact.
 a=np.array(im);w,h=im.size;half=extra_x//2
 assert half%8==0
 xs=list(range(cx))+list(range(cx,cx+8))*(half//8)+list(range(cx,w-cx))+list(range(w-cx,w-cx+8))*(half//8)+list(range(w-cx,w))
 ys=list(range(cy))+list(range(cy,cy+8))*(extra_y//8)+list(range(cy,h))
 return Image.fromarray(a[np.array(ys)[:,None],np.array(xs)[None,:]])
...
levels=[('ss2','Sous-sol −2','Spinda',32,16),('ss1','Sous-sol −1','Spinda',32,16),('accueil','Accueil','Metano',32,16),('etage1','Étage +1','Metano',32,16),('etage2','Étage +2','Metano',32,16)]
manifest={'levels':[],'materials':SOURCES,'window_views':'232233.png, paysage canonique fourni précédemment; fenêtres = nouveaux assemblages de poutres natives','native_texture_scale':1,'grid_px':8,'runtime_validated':False,'furniture':False,'access_policy':'Original threshold and its lighting copied without resizing; no invented floor links'}
for variant in ['A','B']:
 for index,(slug,title,family,ex,ey) in enumerate(levels):
  ex+=16 if variant=='B' else 0;ey+=8 if variant=='B' else 0;underground=family=='Spinda';src=spempty if underground else metano;fm=spmask if underground else mfloor;cx,cy=(200,224) if underground else (128,192)
  ...
  # Restore the COMPLETE canonical threshold, including original light/shadow.
  # No additional stairs and no invented inter-floor connections.
  reference=load(P/('spinda_cafe_reference.png' if underground else 'metano_cafe_reference.png'))
  entry_box=(280,320,416,408) if underground else (256,280,328,320)
  x0,y0,x1,y1=entry_box;dx=ex//2;dy=ey
  stairs=Image.new('RGBA',room.size)
  stairs.paste(reference.crop(entry_box),(x0+dx,y0+dy))
  # Opaque background inside this original crop deliberately preserves shadow RGB.
  ports=[{'direction':'accès original','xy':[(x0+x1)//2+dx,(y0+y1)//2+dy],'target':'extérieur / seuil canonique','source_box':list(entry_box),'destination_box':[x0+dx,y0+dy,x1+dx,y1+dy]}]
  # Full visible shell remains editable; exterior backdrops only occupy actual apertures.
  common={'00_fond_canonique':Image.new('RGBA',room.size,reference.getpixel((0,0))),'01_sol_vide':Image.fromarray(floor),'03_bois_murs':Image.fromarray(wood),'04_bordure_rocheuse':Image.fromarray(rock),'05_cadres_fenetres':frames,'06_escaliers':stairs}
  ...
  for mode,view in [('jour',views_day),('nuit',views_night)]:
   ...
   for n in ['00_fond_canonique','01_sol_vide','02_vues','03_bois_murs','04_bordure_rocheuse','05_cadres_fenetres','06_escaliers']:comp.alpha_composite(view if n=='02_vues' else common[n])
...
print('10 modestly enlarged rooms: original entrances and lighting preserved, circular windows')
