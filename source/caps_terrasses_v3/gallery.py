...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'renders/caps_terrasses_v3';SIZE=(820,328)
BASE='https://github.com/meromoonmeri/guilde-treehouse-pmd/blob/arena/01a095e8-guilde-treehouse-pmd/renders/caps_terrasses_v3/'
...
def view(p):return uri(Image.open(p).convert('RGBA').resize(SIZE,Image.Resampling.NEAREST))
m=json.loads((OUT/'manifest.json').read_text());zones=[]
for z in m['zones']:
 zones.append({'title':z['title'],'day':view(OUT/z['terrain']),'night':view(OUT/z['night']),'link':BASE+z['terrain'],'nightLink':BASE+z['night'],'sceneLink':BASE+z['scene'],'sceneNightLink':BASE+z['scene_night'],'size':z['size']})
sea=Image.open(OUT/'ocean/jour_00.png') # actual palette indices, never RGB luminance
import numpy as np
...
pal={mode:[Image.open(OUT/'ocean'/f'{mode}_{i:02d}.png').getpalette()[:active*3] for i in range(64)] for mode in ['jour','nuit']}
data={'zones':zones,'indices':uri(indices,'PNG'),'alpha':alpha,'palettes':pal,'sky':{'jour':view(OUT/'fonds/ciel.png'),'nuit':view(OUT/'fonds/ciel_nuit.png')},'clouds':{'jour':view(OUT/'fonds/nuages.png'),'nuit':view(OUT/'fonds/nuages_nuit.png')}}
...
function controls(){dirty=true;const z=DATA.zones[+$('zone').value];$('png').href=$('night').checked?z.nightLink:z.link;$('scene').href=$('night').checked?z.sceneNightLink:z.sceneLink;$('size').textContent=z.size.join(' × ')+' px · PNG RGBA · terrain sans fond · aperçu réduit à 820 × 328'}
for(const [i,z] of DATA.zones.entries())$('zone').add(new Option(z.title,i));
...
(ROOT/'apercu_caps_terrasses_v3.html').write_text(page)
