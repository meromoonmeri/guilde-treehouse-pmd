...
(async()=>{
 ...
 const html=fs.readFileSync('apercu_caps_terrasses_v4.html','utf8'),script=html.match(/<script>([\s\S]*)<\/script>/)[1];vm.runInContext(script,context);await new Promise(setImmediate);assert(next);next(100);next(150);assert.equal(vm.runInContext('lastPhase',context),1);
 assert.equal($('zone').children.length,10);
 for(let i=0;i<10;i++)for(const night of [false,true]){$('zone').value=String(i);$('night').checked=night;$('zone').listeners.change();next(150);assert($('png').href.endsWith(night?'_terrain_nuit.png':'_terrain.png'));assert($('audit-status').textContent.startsWith([0,4].includes(i)?'NON RETENU':'INSPECTÉ'))}
 ...
 console.log('PASS: 20 terrain/ambience choices, four visibility controls, pause/resume, 64-phase and legacy clocks; simulated DOM only.');
