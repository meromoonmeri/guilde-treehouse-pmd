...
def main():
    ...
    assert header.findtext('Namespace')=='cotes_v2_0812' and header.findtext('GameVersion')=='0.8.12.0'
    ...
    for mode in ['jour','nuit']:
        for kind in ['ciel','astres','nuages']:
            assert np.array_equal(np.array(backgrounds[f'V30812_{mode.upper()}_{kind.upper()}']),V.expected(WEB/'fonds'/f'{mode}_{kind}.png'))
    ...
    for zone in zones:
        ...
        crop=zone['crop'];rock=np.array(Image.open(base/'01_ROCHE_METANO.png'));mask=rock[:,:,3]>0
        yy,xx=np.indices((size[1],size[0]));expected=np.array(control)[(yy+crop[1])%48,(xx+crop[0])%64]
        assert np.array_equal(rock[mask],expected[mask]),'Native rock material changed'
        terrain=np.array(Image.open(base/'TERRAIN.png'));before=np.array(Image.open(base/'AVANT_ROCHE.png'))
        ...
        for mode,variant in zone['variants'].items():
            ...
            assert o['AssetName']==variant['asset'] and o['ActiveChar'] is None and len(layers)==8
            for layer in layers:
            ...
            assert (PACK/f'Data/Script/cotes_v2_0812/ground/{variant["asset"]}/init.lua').is_file()
            bgs=o['Background']['Layers'];assert o['Background']['$type']=='RogueEssence.Dungeon.LayeredBG, RogueEssence'
            assert [b['BG']['BGAnim']['AnimIndex'] for b in bgs]==[f'V30812_{mode.upper()}_{k}' for k in ['CIEL','ASTRES','NUAGES']]
    ...
    report={'status':'PASS','target':'0.8.12','native_runtime_tested':False,'maps':20,
            'native_premultiplied_layers_checked':80,'sea_frames_checked':16,'native_backgrounds':6,
            ...
            'Generated grass and fringe are not canonical tiles','Night is graded; shadows are separate from raw canonical day rock',
