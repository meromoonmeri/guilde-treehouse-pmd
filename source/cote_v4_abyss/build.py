...
def pipeline():
    ...
    b.WEB=WEB;b.OUT=PACK;b.SIZE=(1168,912);b.SHEET_PREFIX='V40812_';b.MAP_PREFIX='v40812_'
...
def main():
    ...
    banks={mode:{'sea':b.N.TileBank('V40812_'+mode.upper()+'_MER'),
                 'terrain':b.N.TileBank('V40812_'+mode.upper()+'_TERRAIN')} for mode in ['jour','nuit']}
    phases=[]
    for i in range(8):
    ...
    labels=['Herbe Metano native','Faces Metano natives','Retours Metano natifs','Couronnes Metano natives','Pieds Metano natifs']
    for rec in prep['zones']:
        ...
        entry={'id':slug,'title':rec['title'],'size':size,'new':True,'entry':[x-8,y-8],
               ...
               'variants':{},'terrain_provenance':'approved silhouette masks; all terrain RGB from native Metano modules; exact Abyss night filter'}
        for mode in ['jour','nuit']:
            layers=[(label,b.grade(im,mode)) for label,im in planes]
            sea=[im.crop((0,0,*size)) for im in seas[mode]]
            asset='v40812_'+slug+'_'+mode
            doc=b.make_map(asset,rec['title'],mode,layers,sea,banks[mode],template)
            doc['Version']='0.8.12.0'
            o=doc['Object']
            o['Comment']='PMDO 0.8.12. Silhouette V2; modules Metano natifs; filtre nuit Abyss exact. Bords W/E/S sans marge. Collisions a dessiner.'
    ...
    shutil.copyfile(HERE/'provenance.json',PACK/'provenance/abyss_sources.json')
    shutil.copyfile(HERE/'tile_night_reference.py',PACK/'provenance/tile_night_reference.py')
