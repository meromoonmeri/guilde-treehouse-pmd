...
import sys
sys.path.append(str(Path(__file__).resolve().parents[1]/"cote_v4_abyss"))
...
SOURCE=ROOT/'source/cote_v4_abyss'
...
def audit():
 ...
 raw=(SOURCE/'tile_night_reference.py').read_bytes()
 assert hashlib.sha1(b'blob '+str(len(raw)).encode()+b'\0'+raw).hexdigest()=='438383f479e2d80a6a0b3be4cced4087470d9835'
 banks={k:decode(SOURCE/'natifs'/f'Metano_Town_{k}.tile') for k in ['Base','Cliffs','Fringe']}
 ...
 for k,a in banks.items():
  assert np.array_equal(np.array(night(Image.fromarray(a))),decode(SOURCE/'natifs'/f'Metano_Town_{k}_Night.tile'))
 prep=json.loads((WEB/'preparation.json').read_text());count=0
 for z in prep['zones']:
  base=WEB/z['id']
  land=np.array(Image.open(base/'MASQUE_TERRE.png'))>0
  grass=np.array(Image.open(base/'MASQUE_HERBE.png'))>0
  ...
  for rec in records:
   ...
   mask=(grass if layer==0 else land if layer==5 else land&~grass)[t:b,l:r]&(src[:,:,3]==255)
   if rec['module'] in ['couronne','portail']:
