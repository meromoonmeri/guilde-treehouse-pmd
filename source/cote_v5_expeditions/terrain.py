...
ROOT=HERE.parents[1]
OUT=ROOT/'sprites/cote_v5_expeditions'
NATIVE=ROOT/'source/cote_v4_abyss/natifs'
NAMES=['00_HERBE_NATIVE.png','01_FACES_NATIVE.png','02_RETOURS_NATIVE.png','03_COURONNES_NATIVE.png','04_PIEDS_NATIVE.png','05_ACCES_NATIF.png']
MODULES={'herbe':('Base',(0,640,128,768)), 'face':('Cliffs',(912,464,976,512)),
 ...
 'pied':('Cliffs',(912,528,976,544)), 'portail':('Cliffs',(840,416,912,544)), 'passage':('Cliffs',(680,448,744,544))}
...
def main():
 ...
 sheets={name:decode(NATIVE/f'Metano_Town_{name}.tile') for name in ['Base','Cliffs','Fringe']}
 ...
 for name,day in sheets.items():
  actual=decode(NATIVE/f'Metano_Town_{name}_Night.tile')
  assert np.array_equal(np.array(night(Image.fromarray(day))),actual),name
 prep=json.loads((OUT/'layouts.json').read_text())
 prep['rock_source']=json.loads((ROOT/'sprites/cote_v4_abyss/preparation.json').read_text())['rock_source']
 ...
 for rec in prep['zones']:
  slug=rec['id'];dest=OUT/slug
  land=np.array(Image.open(dest/'MASQUE_TERRE.png'))>0
  grass=np.array(Image.open(dest/'MASQUE_HERBE.png'))>0
  original=np.zeros((*land.shape,4),dtype='uint8');original[:,:,3]=land*255
  ...
  if rec['door']:
   door=rec['door'];x,y=door['position'];stamp(5,door['module'],x,y,land)
   if door['module']=='passage':stamp(5,'passage',648,y,land)
  ...
  for p in placements:
   ...
   select=(grass if layer==0 else land if layer==5 else rock)[t:b,l:r]&(src[:,:,3]==255)
   if p['module'] in ['couronne','portail']:
