...
sys.path.append(str(ROOT/'source/cote_v4_abyss'))
...
def main():
    ...
    assert header.findtext('Namespace')=='metano_expeditions' and header.findtext('GameVersion')=='0.8.12.0'
    ...
    for mode in ['jour','nuit']:
        for kind in ['ciel','astres','nuages']:
            assert np.array_equal(np.array(backgrounds[f'V50812_{mode.upper()}_{kind.upper()}']),V.expected(WEB/'fonds'/f'{mode}_{kind}.png'))
    ...
    for zone in zones:
        ...
        terrain=np.array(Image.open(base/'TERRAIN.png'));before=terrain.copy();before[:,:,3]=np.array(Image.open(base/'MASQUE_TERRE.png'))
        ...
        for mode,variant in zone['variants'].items():
            ...
            assert o['AssetName']==variant['asset'] and o['ActiveChar'] is None and len(layers)==10
            for layer in layers:
            ...
            grass=(np.array(Image.open(base/'MASQUE_HERBE.png'))>0)&(np.array(Image.open(base/'05_ACCES_NATIF.png'))[:,:,3]==0)
            assert len(o['obstacles'])==w and all(len(c)==h for c in o['obstacles'])
            for x,col in enumerate(o['obstacles']):
                for y,t in enumerate(col):assert t=={'Bounds':{'X':8*x,'Y':8*y,'Width':8,'Height':8},'Tags':0 if grass[y*8:y*8+8,x*8:x*8+8].all() else 1}
            ...
            assert (PACK/f'Data/Script/metano_expeditions/ground/{variant["asset"]}/init.lua').is_file()
            bgs=o['Background']['Layers'];assert o['Background']['$type']=='RogueEssence.Dungeon.LayeredBG, RogueEssence'
            assert [b['BG']['BGAnim']['AnimIndex'] for b in bgs]==[f'V50812_{mode.upper()}_{k}' for k in ['CIEL','ASTRES','NUAGES']]
    ...
    with tempfile.TemporaryDirectory(prefix='v30812_install_') as tmp:
        ...
        assert len(list((target/'Data/Script/test_existing/ground').glob('*/init.lua')))==40
    ...
    report={'materials':material_report,'status':'PASS','target':'0.8.12','native_runtime_tested':False,'maps':40,'new_maps':20,
            'new_native_premultiplied_layers_checked':120,'sea_frames_checked':16,'native_backgrounds':6,
            ...
            'New obstacle grid blocks non-grass; review terrace connections in game; old maps preserve their original free collisions','Dungeon threshold markers supplied, no dungeon destination bound']}
