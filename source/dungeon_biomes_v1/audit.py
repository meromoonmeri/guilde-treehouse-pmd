...
def main():
 ...
 files=[]
 with zipfile.ZipFile(HERE/'native_sources.zip','w',zipfile.ZIP_DEFLATED,compresslevel=9) as z:
 ...
 (HERE/'audit.json').write_text(json.dumps(result,indent=2)+'\n');print(result['H_bank_comparison']);print([(r['map'],r['weather_map']) for r in records]);print('native source archive', (HERE/'native_sources.zip').stat().st_size)
def bootstrap():
 import subprocess
 prior=json.loads((HERE/'audit.json').read_text());C.mkdir(parents=True,exist_ok=True)
 with zipfile.ZipFile(HERE/'native_sources.zip') as z:z.extractall(P)
