...
.venv/bin/python source/entree_cratere_sud_nord_v1/package.py   (après build.py)
"""
...
def main():
    subprocess.run([sys.executable, '-m', 'unittest', 'source.entree_cratere_sud_nord_v1.test_build'], cwd=R, check=True)
    M = json.loads((O / 'manifest.json').read_text())
    zipdir(O / 'ECN1_projet_pmdo_0812.zip',
           [(p, 'entree_cratere_sud_nord/' + p.relative_to(S).as_posix()) for p in sorted(S.rglob('*')) if p.is_file()])
    items = [(p, 'calques/' + p.name) for p in sorted((O / 'calques').glob('*.png'))]
    for sub in ['lave', 'eclats', 'bulles', 'braises']:
    ...
    items += [(O / 'ECN1_entree_cratere_calques.ora', 'ECN1_entree_cratere_calques.ora'), (O / 'manifest.json', 'manifest.json'),
    ...
    counts = {'lave': M['lava']['phases'], 'eclats': M['lava']['phases'], 'bulles': M['bubbles']['phases'], 'braises': M['embers']['phases']}
    ticks = {'lave': M['lava']['frame_length_ticks'], 'eclats': M['lava']['frame_length_ticks'],
             'bulles': M['bubbles']['frame_length_ticks'], 'braises': M['embers']['frame_length_ticks']}
    stack = []
    for p in M['layer_order_bottom_to_top']:
        ...
        if kind:
            ...
            stack.append({'id': Path(p).stem.replace('ECN1_', '').rsplit('_f', 1)[0], 'ticks': ticks[kind],
    ...
    data = {'size': M['size_px'], 'loop': M['scene_loop_ticks'], 'stack': stack,
            'surface': 'rgb(%d,%d,%d)' % tuple(M['lava']['couleurs']['surface']),
            'poses': [{'id': k, 'uri': uri(O / f'poses_bulles/ECN1_bulle_{k}.png')} for k in M['bubbles']['poses']],
    ...
    (R / 'apercu_entree_cratere_sud_nord_v1.html').write_text(page)
    for p in [O / 'ECN1_projet_pmdo_0812.zip', O / 'ECN1_calques_png_8px.zip', R / 'apercu_entree_cratere_sud_nord_v1.html']:
