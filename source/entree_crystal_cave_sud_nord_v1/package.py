...
.venv/bin/python source/entree_crystal_cave_sud_nord_v1/package.py   (après build.py)
"""
...
def main():
    subprocess.run([sys.executable, '-m', 'unittest', 'source.entree_crystal_cave_sud_nord_v1.test_build'], cwd=R, check=True)
    M = json.loads((O / 'manifest.json').read_text())
    zipdir(O / f'{PFX}_projet_pmdo_0812.zip',
           [(p, 'entree_crystal_cave_sud_nord/' + p.relative_to(S).as_posix()) for p in sorted(S.rglob('*')) if p.is_file()])
    ...
    items += [(O / f'{PFX}_entree_crystal_cave_calques.ora', f'{PFX}_entree_crystal_cave_calques.ora'),
    ...
    poses = []
    for kind, n in (('eclat_cyan', 4), ('eclat_rose', 4)):
        for i in range(n):
            p = O / f'poses/{PFX}_{kind}_{i}.png'; w, h = Image.open(p).size
            poses.append({'id': f'{kind} {i}', 'uri': uri(p), 'w': w, 'h': h})
    data = {'size': M['size_px'], 'loop': M['scene_loop_ticks'], 'stack': stack, 'surface': 'rgb(0,63,111)', 'poses': poses,
    ...
    (R / 'apercu_entree_crystal_cave_sud_nord_v1.html').write_text(page)
    for p in [O / f'{PFX}_projet_pmdo_0812.zip', O / f'{PFX}_calques_png_8px.zip', R / 'apercu_entree_crystal_cave_sud_nord_v1.html']:
