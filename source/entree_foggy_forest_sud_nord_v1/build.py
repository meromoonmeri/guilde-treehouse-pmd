"""Entrée Foggy Forest — camp de base sud -> nord V1 (EFF1) — format 4:3 vaste (768 x 576 px, 96 x 72 cases).

Demande : « oui enchaîne avec Foggy Forest Base Camp mais n'oublie pas que la méthode doit être employée ».
Référence : `Foggy_Forest_Base_Camp_TDS.png` (PMD Explorers), jamais utilisée comme référence principale.
...
- sol_complet.png : herbe pâle complète éditée depuis le décor (premier essai, aucune réparation) ;
- brume_poses.png : planche de 8 nappes de brume sur magenta (grille 2 x 4 demandée, rendue 4 x 2).
Calques : sol complet, herbe du camp, chemin, sous-bois (herbe sombre), buissons et fleurs, rochers, tentes,
arbres, grotte (encadrement de pierre et bouche sombre).
...
- brume : 8 nappes générées réduites, tramées en damier fixe (alpha 0/255 uniquement), dérive sinusoïdale fermée,
  48 x 5 ticks (animation créée par nous).
Scène : PPCM(40, 240) = 240 ticks = 4 s.
Lancer : .venv/bin/python source/entree_foggy_forest_sud_nord_v1/build.py
"""
...
OUT = R / 'renders/entree_foggy_forest_sud_nord_v1'
STAGE = R / '.cache/entree_foggy_forest_sud_nord_v1/entree_foggy_forest_sud_nord'
NAMESPACE = 'entree_foggy_forest_sud_nord'
ASSET = 'eff1_entree_foggy_forest'
PFX = 'EFF1'
W, H = 768, 576
SRC = (1200, 896)
...
PALETTE_GROUPS = {'terrain': (['sol_complet', 'herbe', 'chemin', 'sous_bois', 'buissons'], 96),
                  'arbres': (['arbres'], 40), 'tentes': (['tentes'], 32), 'rochers': (['rochers'], 16),
                  'grotte': (['grotte'], 16)}
...
def ground_project(stack, blocked, entry_px, threshold_px, gfx, tools):
    ...
    o.update(Name={'DefaultText': 'Entree Foggy Forest - camp de base, sud vers nord (4:3)', 'LocalTexts': {}}, AssetName=ASSET,
...
<Header>
  <Name>Entree Foggy Forest camp sud-nord 4:3 - Atelier 0.8.12</Name>
  <Author>meromoonmeri</Author>
  <Description>Projet d'edition : camp de base de foret genere au format 4:3 (ref. rip Foggy Forest Base Camp), mare facon Metano, brume animee. Pas une aventure jouable.</Description>
...
def write_ora(path, layers):
    ...
    root = ET.Element('image', w=str(W), h=str(H), name='Entree Foggy Forest camp sud-nord V1 (EFF1)')
...
def build():
    ...
    m = classify(a); mouth_full = m.pop('bouche')
    order = ['water', 'grotte', 'tentes', 'chemin', 'rochers', 'buissons', 'arbres', 'sous_bois', 'herbe']
    ...
    write_ora(OUT / f'{PFX}_entree_foggy_forest_calques.ora',
    ...
    fid = fidelity(a, ref)
    final_fid = {}
    for k, nm in (('herbe_camp', 'herbe'), ('chemin', 'chemin'), ('sous_bois', 'sous_bois'), ('tentes', 'tentes')):
    ...
    manifest = {
        'lot': 'entree_foggy_forest_sud_nord_v1', 'prefix': PFX, 'format': '4:3 vaste', 'size_px': [W, H],
        'grid_8px': [W // 8, H // 8], 'base': 'branche de session (EMF1 pour les utilitaires) ; aucun emprunt aux branches soeurs',
        'biome': 'Foggy Forest Base Camp, demande par l utilisateur',
