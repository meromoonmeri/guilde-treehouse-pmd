...
.venv/bin/python source/entree_foggy_forest_sud_nord_v1/package.py   (après build.py)
"""
...
def main():
    subprocess.run([sys.executable, '-m', 'unittest', 'source.entree_foggy_forest_sud_nord_v1.test_build'], cwd=R, check=True)
    M = json.loads((O / 'manifest.json').read_text())
    zipdir(O / f'{PFX}_projet_pmdo_0812.zip',
           [(p, 'entree_foggy_forest_sud_nord/' + p.relative_to(S).as_posix()) for p in sorted(S.rglob('*')) if p.is_file()])
    ...
    items += [(O / f'{PFX}_entree_foggy_forest_calques.ora', f'{PFX}_entree_foggy_forest_calques.ora'),
    ...
    (R / 'apercu_entree_foggy_forest_sud_nord_v1.html').write_text(page)
    for p in [O / f'{PFX}_projet_pmdo_0812.zip', O / f'{PFX}_calques_png_8px.zip', R / 'apercu_entree_foggy_forest_sud_nord_v1.html']:
