...
.venv/bin/python source/entree_southern_jungle_sud_nord_v1/package.py   (après build.py)
"""
...
def main():
    subprocess.run([sys.executable, '-m', 'unittest', 'source.entree_southern_jungle_sud_nord_v1.test_build'], cwd=R, check=True)
    M = json.loads((O / 'manifest.json').read_text())
    zipdir(O / f'{PFX}_projet_pmdo_0812.zip',
           [(p, 'entree_southern_jungle_sud_nord/' + p.relative_to(S).as_posix()) for p in sorted(S.rglob('*')) if p.is_file()])
    ...
    items += [(O / f'{PFX}_entree_southern_jungle_calques.ora', f'{PFX}_entree_southern_jungle_calques.ora'),
    ...
    poses = []
    for kind, n in (('papillon_jaune', 4), ('papillon_bleu', 4)):
        for i in range(n):
    ...
    (R / 'apercu_entree_southern_jungle_sud_nord_v1.html').write_text(page)
    for p in [O / f'{PFX}_projet_pmdo_0812.zip', O / f'{PFX}_calques_png_8px.zip', R / 'apercu_entree_southern_jungle_sud_nord_v1.html']:
