...
.venv/bin/python source/entree_waterfall_cave_sud_nord_v2/package.py   (après build.py)
"""
...
REVIEW = ['scene_fermee_t000.png', 'scene_ouverte_t000.png', 'scene_animee.webp', 'ouverture_planche_x2.png',
          'collisions_marqueurs.png', 'rive_avant_apres_x4.png']
...
def main():
    subprocess.run([sys.executable, '-m', 'unittest', 'source.entree_waterfall_cave_sud_nord_v2.test_build'], cwd=R, check=True)
    M = json.loads((O / 'manifest.json').read_text())
    zipdir(O / f'{PFX}_projet_pmdo_0812.zip',
           [(p, 'entree_waterfall_cave_v2/' + p.relative_to(S).as_posix()) for p in sorted(S.rglob('*')) if p.is_file()])
    ...
    (R / 'apercu_entree_waterfall_cave_sud_nord_v2.html').write_text(page)
    for p in [O / f'{PFX}_projet_pmdo_0812.zip', O / f'{PFX}_calques_png_8px.zip', R / 'apercu_entree_waterfall_cave_sud_nord_v2.html']:
