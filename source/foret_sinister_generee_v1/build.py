...
Entree : renders/foret_sinister_generee_v1/bruts/foret_sinister_complete.png (848x1264, genere).
...
# lissage Jacobi du remplissage (frontieres herbe/chemin fixes) + grain
cur = sol.astype(float)
fx = hole.copy()
from scipy.ndimage import uniform_filter as _uf
for _ in range(30):
    for c in range(3):
        sm = _uf(cur[:, :, c], 5, mode="nearest")
        cur[:, :, c][fx] = sm[fx]
cur[fx] += rng.integers(-4, 5, size=(int(fx.sum()), 3))
sol = np.clip(cur, 0, 255).round().astype(int)
