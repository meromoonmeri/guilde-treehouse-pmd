...
BR = R / 'renders/forets_lisiere_v1/bruts'
OUT = R / 'renders/forets_lisiere_v1'
EXP = R / 'exports/forets_lisiere_v1'
sys.path.insert(0, str(R / 'source/cote_v4_abyss'))
from night import night  # noqa: E402

W, H, T = 928, 1152, 8
MAPS = [
    {'id': 'A_clairiere_grotte_nord', 'title': 'Clairière — grotte dans la lisière nord',
     'scene': 'A_clairiere_grotte_nord.png', 'arrival': 'sud', 'objective': 'entrée grotte nord (lisière)'},
    {'id': 'B_foret_entree_ouest', 'title': 'Forêt — entrée de donjon dans la lisière ouest',
     'scene': 'B_foret_entree_ouest.png', 'arrival': 'sud', 'objective': 'grotte ouest (lisière)'},
    {'id': 'C_caverne_verdoyante', 'title': 'Caverne verdoyante — ouverture sud vers la forêt',
     'scene': 'C_caverne_verdoyante.png', 'arrival': 'sud (lisière)', 'objective': 'fond de grotte nord'},
    {'id': 'D_fond_foret', 'title': 'Fond de forêt — pièce de lisière, chemin sud→nord',
     'scene': 'D_fond_foret.png', 'arrival': 'sud', 'objective': 'lisière nord'},
]
...
def split_scene(rgb: np.ndarray, kind: str):
    ...
    cave &= ~green
    if kind == 'D_fond_foret':
        cave[:] = False
        ochre = ochre & dirt  # few rocks
    if kind == 'C_caverne_verdoyante':
        dark_f = green & (g < 140)  # moss on walls treated as lisiere/veg
        # walls = ochre or dark non-green
        ochre = (~green) & (mx > 40)
        cave = dark & ~green
    ...
    path = nd.binary_closing(dirt & ~ochre, iterations=2) if kind != 'C_caverne_verdoyante' else nd.binary_closing(dirt, iterations=2)
    if kind == 'C_caverne_verdoyante':
...
def build():
    ...
    for spec in MAPS:
        ...
        if mid == 'A_clairiere_grotte_nord':
            overlay_magenta(layers, BR / 'A_calque_lisiere.png', '02_lisiere_foret')
            overlay_magenta(layers, BR / 'A_calque_chemin.png', '03_chemin')
            overlay_magenta(layers, BR / 'A_calque_entree.png', '05_entree_grotte')
            overlay_magenta(layers, BR / 'A_calque_arbres.png', '09_canopees')
        if mid == 'B_foret_entree_ouest':
            overlay_magenta(layers, BR / 'B_calque_entree_ouest.png', '05_entree_grotte')
        if mid == 'C_caverne_verdoyante':
            overlay_magenta(layers, BR / 'C_calque_parois.png', '04_parois_roche')
        # force opaque sol
        ...
        gallery_maps.append({
            ...
            'layers': [{'id': n, 'jour': f'renders/forets_lisiere_v1/{mid}/calques/{n}.png',
                        'nuit': f'renders/forets_lisiere_v1/{mid}/nuit/{n}_nuit.png'} for n in ORDER],
        })
        index.append(mid)
        print('OK', mid)
    ...
    (R / 'apercu_forets_lisiere_v1.html').write_text(html)
    # planche 0.25x
    ...
    board.resize((board.width // 4, board.height // 4), Image.NEAREST).save(OUT / 'planche_4maps_0.25x.png')
    print('4 maps')
