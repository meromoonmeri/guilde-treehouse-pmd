...
const out=path.resolve(__dirname,'../../renders/ice_arena_aurora_coherent_v3');
...
async function main(){
 ...
 check('Preserved outer sky and one generated terrain layer exposed',ids.terrain.children.length===2&&ids.scene.calls.length===3);
 ...
 ids.restore.emit('click');check('Restore re-enables all seven displayed layers',document.querySelectorAll().every(c=>c.checked)&&ids.bg.calls.length===5);
 ...
 check('Color-port and runtime limitations remain visible',html.includes('Terrain généré, pas des tuiles natives certifiées')&&html.includes('Pas de validation GPU PMDO'));
