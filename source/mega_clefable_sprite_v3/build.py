...
R=Path(__file__).resolve().parents[2];S=Path(__file__).resolve().parent;G=S/'gen';REF=R/'source/mega_clefable_sprite_v1/references/spritecollab_0036';O=R/'renders/mega_clefable_sprite_v3';OUT=O/'sprite/0036/0001';OUT.mkdir(parents=True,exist_ok=True)
guides=json.load(open(G/'guides.json'))
...
for an in list(anims_el):
    ...
    cw,ch=W/cols,H/rows;cells={}
    for comp in comps:
        c=min(cols-1,max(0,int(comp['cx']//cw)));r=min(rows-1,max(0,int(comp['cy']//ch)))
        key=(r,c)
...
with zipfile.ZipFile(O/'0036_0001_mega_clefable_v3_spritecollab.zip','w',zipfile.ZIP_DEFLATED) as z:
...
(O/'manifest.json').write_text(json.dumps(dict(base_xml_offsets_shadows='CHUNSOFT sprite/0036 @aae4cee2 (unchanged)',palette=PAL.tolist(),anims=report,missing_anims=missing,raw_sheets={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(G.glob('raw_*.png'))},generated=True,runtime_validated=False),indent=1))
