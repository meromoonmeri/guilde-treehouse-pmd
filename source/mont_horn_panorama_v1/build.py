...
R=Path(__file__).resolve().parents[2];O=R/'renders/mont_horn_panorama_v1';P=O/'mont_horn';S=O/'sprites';P.mkdir(parents=True,exist_ok=True);S.mkdir(exist_ok=True)
...
bg=load(O/'bruts/panorama.png').resize((W,H),NN);bg=bg.convert('RGB').quantize(colors=80,method=Image.Quantize.MEDIANCUT).convert('RGBA')
fg0=key(load(O/'bruts/relief_magenta.png')).resize((432,H),NN);fg=Image.new('RGBA',(W,H));fg.alpha_composite(fg0,(108,0));fa=np.array(fg);visible=fa[:,:,3]>0
...
cloudsheet=key(load(O/'bruts/nuages_magenta.png'));ca=np.array(cloudsheet);rowcount=(ca[:,:,3]>0).sum(axis=1);active=rowcount>12;spans=nd.find_objects(nd.label(active)[0]);assert len(spans)==4,len(spans)
clouds=[]
...
def layers(phase):
 c=cloud_layers(phase);return [('01_panorama_montagnes',bg),c[0]]+terrain+[c[1]]
for name,im in [('01_panorama_montagnes',bg)]+terrain:im.save(P/f'horn_{name}.png')
frames=[]
for k in range(N):
 ls=layers(k)
...
m={'size':[W,H],'reference':'Mt_Horn_entrance_Sky.png','source_size':list(load(R/'Mt_Horn_entrance_Sky.png').size),'art_origin':'AI-generated reference-guided redesign, not native sprite recovery','foreground_resize':[432,H],'foreground_origin':[108,0],'static_layers':['01_panorama_montagnes']+[n for n,_ in terrain],'animated_groups':['02_nuages_lointains','07_nuages_overlay'],'frames':N,'frame_ms':DT,'loop_ms':N*DT,'wrap_width':W,'shift_pixels_per_frame':[2,4],'velocities_px_second':[6.25,12.5],'overlay_blend':'normal source-over, alpha <=69, spatial fade to zero over path/cave','route_guide':route,'segmentation':'disjoint masks partition generated foreground; independent aligned scene layers, not invented hidden surfaces','runtime_validated':False}
