"""Mont Thunder V8 — falaise sans bordure de chemin + brume électrostatique en contrebas.
...
OUT = ROOT / "renders" / "mont_thunder_orage_v8"
PFX = "MTTHUNDER_V8_"
W, H = 304, 456
...
# ---------- terrain ----------
g = Image.open(HERE / "generation" / "falaise_guide_sans_bordure.png")
...
bg = Image.open(ROOT / "source" / "mont_thunder_orage_v7" / "generation" / "brume_guide.png")
...
# ---------- brume au rebord rocheux (physique : ressac de vapeur contre les parois) ----------
# La brume du contrebas vient lécher le pied des parois sombres : la hauteur de la langue de
# vapeur suit deux ondes progressives le long du contour (paramètre = abscisse curviligne
# approchée par x + y), qui montent puis retombent ; bord tramé Bayer 4x4 (pixel art, alpha binaire).
NF, FMS = 24, 120
Lum = terr[..., :3].astype(int).mean(2)
void_all = terr[..., 3] == 0
face = (terr[..., 3] > 0) & (Lum < 80) & ~np.kron(grid, np.ones((8, 8), bool))
din = nd.distance_transform_edt(~void_all)          # distance au vide, dans la roche
yy, xx = np.mgrid[:H, :W]
zone = face & (yy > BR_Y + 16) & (din < 20)
BAYER = np.array([[0, 8, 2, 10], [12, 4, 14, 6], [3, 11, 1, 9], [15, 7, 13, 5]]) / 16.0
bay = np.tile(BAYER, (H // 4 + 1, W // 4 + 1))[:H, :W]
tex = np.zeros((H, W, 3), int)
reps = int(np.ceil((H - BR_Y) / BH)) + 1
tex[BR_Y:] = np.concatenate([brume[..., :3].astype(int)] * reps, 0)[:H - BR_Y, :W]
tex = np.minimum(248, (tex + 16) // 8 * 8)          # un pas plus clair : vapeur au premier plan
s_ = xx + yy
def rebord(f):
    ph = f / NF
    h = 7 + 5 * np.sin(2 * np.pi * (s_ / 72.0 - ph)) + 3 * np.sin(2 * np.pi * (s_ / 29.0 + 2 * ph) + 1.1)
    a = np.zeros((H, W, 4), np.uint8)
    solid = zone & (din < h - 3)
    fringe = zone & (din >= h - 3) & (din < h) & (bay < (h - din) / 3.0)
    m = solid | fringe
    a[m, :3] = tex[m]; a[m, 3] = 255
    return a
REB = [rebord(f) for f in range(NF)]
# ---------- sorties ----------
...
LAYERS = [("00_ciel", sky), ("01_nuages", clouds_at(0, 0)), ("02_brume", band_at(brume, 0, BR_Y)),
          ("03_brume_etincelles", band_at(sparks(0), 0, BR_Y)), ("04_eclairs", None), ("05_terrain", terr), ("06_brume_rebord", REB[0])]
for n_, a in LAYERS:
    if a is not None: save(a, OUT / "calques" / f"{n_}.png"); save(a, OUT / "import_png_8px" / f"{PFX}{n_}.png")
for f in range(NF): save(REB[f], OUT / "brume" / f"{PFX}06_brume_rebord_f{f:02d}.png")
...
def compose(t):
    ...
    for a in (sky, clouds_at(t, lv), band_at(brume, t, BR_Y), band_at(sparks(int(t // SPMS) % NSP), t, BR_Y), e, terr, REB[int(t // FMS) % NF]):
...
manifest = {"guides_generes": {p.name: hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted((HERE / "generation").glob("*_guide.png"))},
            ...
            "brume_rebord": {"frames": [f"brume/{PFX}06_brume_rebord_f{f:02d}.png" for f in range(NF)], "ms": FMS, "boucle_exacte": True, "note": "ressac de vapeur au pied des parois (2 ondes progressives)"}, "nuages": {"source": "V6", "y": CL_Y, "vitesse_px_s": -6}, "timeline": timeline,
...
with zipfile.ZipFile(OUT.parent / "mont_thunder_orage_v8_pack.zip", "w", zipfile.ZIP_DEFLATED) as z:
    for p in sorted(OUT.rglob("*")):
        if p.is_file(): z.write(p, p.relative_to(OUT.parent))
    z.write(__file__, "mont_thunder_orage_v8/source/build.py")
