...
- `build_dynamax_sprites.py` : versions **Dynamax** de dix sprites (huit références SpriteCollab + Falinks + Zarude) : toutes les animations de la source, agrandies × 3 pixel par pixel, aura rouge animée (motif tramé qui remonte, langues), repères et ancre replacés à l'échelle ; écrit `personnages/dynamax/<numéro>_<slug>/` (feuilles PNG indexés, `AnimData.xml`, nuit, Aseprite, aperçus, `kit.json` avec l'entrée `dynamax.vfx`, `credits.txt`, README). Option `--nuages-integres` pour cuire les nuages dans les feuilles.
- `dynamax_fx.py` : pièces de pixel art des effets Dynamax à l'échelle 1 (bitmaps ASCII des nuages-cyclones à trois phases et de leurs traînées, aura, rayon, colonne d'énergie, éclairs épais en zigzag / hélice, éclat en étoile, ellipses) ; palette de cinq couleurs.
- `build_dynamax_vfx.py` : **VFX Dynamax génériques**, sans personnage ni fond, en deux tailles (M / L) : `Transformation` (15 images, Hit / Return), `NuagesApparition` (5), `Nuages` (boucle de 12, un tiers de tour par boucle), `Aura` (4) ; écrit `personnages/dynamax/vfx/` (feuilles à une ligne, `AnimData.xml` index 13+, Offsets / Shadow pour l'ancre, Aseprite, `apercu.png`, `apercu_effets.gif`, `apercu_demonstration.gif`, `kit.json`, README).
- `verify_dynamax_sprites.py` : relecture des packs (règles SpriteBot + chaque pixel de la source retrouvé à sa place × 3 sans recoloration, ancre et repères × 3, durées / index / `CopyOf` identiques, palette = source + aura, entrée `dynamax.vfx` cohérente) et du dossier `vfx/` (`--vfx`), écrit `controle_qualite.json`.
