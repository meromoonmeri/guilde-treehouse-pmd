...
SHEETS = {
    'Brine Cave Entrance': 'PLAGE_CV2_BRINE',
}
...
def load_sheets(directory: Path = REF, prefix: str = 'EoSO__') -> dict[str, TileSheet]:
    # Nom local : espaces de la feuille EoSO remplacés par des tirets bas.
    return {name: TileSheet(directory / f"{prefix}{name.replace(' ', '_')}.tile") for name in SHEETS}
