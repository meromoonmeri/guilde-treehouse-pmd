...
ROOT=Path(__file__).resolve().parents[3];SRC=Path(__file__).parent;OUT=ROOT/'exports/pokemon_custom/tirtouga_v4'
...
scene_names={'Scenes_A':['EventSleep','Wake','Eat','DeepBreath','Nod','LookUp'],'Scenes_B':['Tumble','Trip','LostBalance','TumbleBack','HitGround','Faint']}
for sheet,names in scene_names.items():
 bounds=[0,175,350,512,691,854,1024] if sheet=='Scenes_A' else [84,245,408,565,717,869,1024]
 srows=source_rows(sheet,6,4,bounds)
 for rowidx,(name,source) in enumerate(zip(names,srows)):
  ...
  elif name not in ['HitGround','Faint']:source=[ImageOps.mirror(im) for im in source]
  ...
  row=[]
  for f,im in enumerate(source):
   c=register(im,scale,settled=True)
...
plan['generation_limit']=10;plan['actions_exported']=len(animations);plan['actions_missing']=[n for n in plan['actions'] if n not in animations];(SRC/'production_plan.json').write_text(json.dumps(plan,indent=2)+'\n')
