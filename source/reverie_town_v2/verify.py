#!/usr/bin/env python3
"""Reverie Town v2 — vérification de recomposition : chaque .rsground du paquet natif, rendu par tools/pmdo_tiles.py (décodeur
...
PACK = ROOT / 'exports/reverie_town_v2/paquet_natif'
    ...
    for slug in ('NE', 'NO'):
        for mode in ('jour', 'nuit'):
            for phase in range(4):
                out = Path('/tmp') / f'rvt2_{slug}_{mode}_p{phase}.png'
                subprocess.run([PY, str(ROOT / 'tools/pmdo_tiles.py'), 'render',
                                str(PACK / f'Data/Ground/rvt2_{slug.lower()}_{mode}.rsground'),
                                '--sheets', *map(str, SHEETS), '--out', str(out), '--phase', str(phase)],
                               check=True, capture_output=True)
                a = np.array(Image.open(out).convert('RGB')).astype(int)
                b = np.array(Image.open(ROOT / f'exports/reverie_town_v2/RVT2_{slug}/RVT2_{slug}_composite_{mode}_p{phase + 1}.png').convert('RGB')).astype(int)
                ...
                print(f'RVT2_{slug} {mode} phase {phase + 1}: '
