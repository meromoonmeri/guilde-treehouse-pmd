"""Secret Garden network V2: 7 pieces, per-zone material layers, leafy borders, Sky Peak-style flowers.

Layers per room: 01_sol_chemin, 02_vegetation_fond, 03_cimees_arbres, 04_buissons_sous_bois,
05_vegetation_premier_plan, 06_troncs, 07_rochers, 08_fleurs (+4 anim frames),
09_bordure_feuillue, 10_acces_X per port.
Recomposition of 01..08 (+f0) == cut terrain exactly. Border/ports are extras.
"""
...
OPAQUE_DEPTH = 24
STRIP_DEPTH = 32
WIN_W, WIN_D = 104, 88  # cleared port window in border layer
...
manifest = {'reference': 'secretgarden.png (user commit)', 'version': 2,
...
def border_layer(raw_border, dirs):
    im = load(O / 'bordures' / f'{raw_border}.png')
    w, h = im.size
    if w != h:  # pad to square with magenta (never anisotropic)
        s = max(w, h)
...
for slug, title, raw, raw_border, dirs, polygon in specs:
    ...
    # materials (validated thresholds on couloir_ns overlays)
    ...
    sol = pathpoly & rest
    veg = rest & ~sol
    masks = {'01_sol_chemin': sol,
             '02_vegetation_fond': veg & (yy < 200),
             '03_cimees_arbres': veg & (yy >= 200) & (yy < 400) & (G_ >= 115),
             '04_buissons_sous_bois': veg & (yy >= 200) & (yy < 400) & (G_ < 115),
             '05_vegetation_premier_plan': veg & (yy >= 400),
             '06_troncs': trunks,
             '07_rochers': rocks,
             '08_fleurs': flowers}
    layers = {n: part(scene, m) for n, m in masks.items()}
    # animated flowers, Sky Peak cadence
    frames = flower_frames(layers['08_fleurs'])
    out = O / slug
    out.mkdir(exist_ok=True)
    fdir = out / 'fleurs'
    for i, f in enumerate(frames):
        save(f, fdir / f'08_fleurs_f{i}.png')
    layers['09_bordure_feuillue'] = border_layer(raw_border, dirs)
    entries = []
    for d in dirs:
        ...
        layers['10_acces_' + d] = port
        entries.append({'direction': d, 'xy': list(point), 'width': 64, 'layer': '10_acces_' + d + '.png'})
    ...
    base.alpha_composite(layers['09_bordure_feuillue'])
    for n, im in layers.items():
        if n.startswith('10_acces'):
            base.alpha_composite(im)
    save(base, out / 'composition.png')
    save(scene, out / 'terrain_detoure.png')
    # animated composition GIF (flowers only move)
    still = Image.new('RGBA', SIZE)
    for n in ['01_sol_chemin', '02_vegetation_fond', '03_cimees_arbres', '04_buissons_sous_bois',
              '05_vegetation_premier_plan', '06_troncs', '07_rochers']:
        still.alpha_composite(layers[n])
    still.alpha_composite(layers['09_bordure_feuillue'])
    for n, im in layers.items():
        if n.startswith('10_acces'):
    ...
    order = ['01_sol_chemin', '02_vegetation_fond', '03_cimees_arbres', '04_buissons_sous_bois',
             '05_vegetation_premier_plan', '06_troncs', '07_rochers', '08_fleurs',
             '09_bordure_feuillue'] + ['10_acces_' + d for d in dirs]
    ...
    manifest['rooms'].append({'id': slug, 'title': title, 'raw': raw + '.png',
                              'border': raw_border + '.png', 'ports': entries,
                              'layers': list(layers), 'optional': [], 'flower_px': fl,
...
print(f'V2: 7 rooms; exact partitions 01..08; {nports} ports; borders + Sky Peak flowers (4x200ms)')
