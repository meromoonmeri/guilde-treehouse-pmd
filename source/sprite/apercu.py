...
`AnimData.xml` (forme de base, apparence normale). Pour chacune, **toutes ses animations** sont reprises (noms, index,
`CopyOf`, durées, `RushFrame` / `HitFrame` / `ReturnFrame`, déplacements de l'ancre) et transformées en version Dynamax :
...
3. **trois nuages-cyclones rouges** qui tournent au-dessus de la tête (un tiers de tour par cycle d'animation : la
   ...
   petits nuages (gabarit **M**) si le corps fait moins de 24 px de large à l'échelle 1, grands (**L**) sinon ;
...
agrandi à sa place (seuls les pixels recouverts par un nuage de premier plan diffèrent, moins de 12 % du corps), que
...
(reprises ligne à ligne dans `credits.txt` ; colonne « Licence » ci-dessous, « + » = plusieurs auteurs) :
