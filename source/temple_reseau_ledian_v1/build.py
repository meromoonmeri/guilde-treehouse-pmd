...
R=Path(__file__).resolve().parents[2];P=Path(__file__).resolve().parent/'references';O=R/'renders/temple_reseau_ledian_v1';SIZE=(512,512)
...
patch=load(P/'reference_temple.png').crop((140,200,180,232));save(patch,O/'materiaux/sol_raccord_natif.png');p=np.array(patch.resize((64,64),Image.Resampling.NEAREST));stripe=p[:32].copy();stripe[:,:,3]=np.minimum(stripe[:,:,3],np.array([255]*16+[round(255*(31-y)/16) for y in range(16,32)],dtype='uint8')[:,None])
ladder=load(P/'reference_temple.png').crop((160,140,200,200));save(ladder,O/'materiaux/echelle_native.png')
manifest={'reference':'IMG_5004.png temple 408x408 - dalles claires / murs violets / bordure rocheuse - DA preserve','sha256':__import__('hashlib').sha256((P/'reference_temple.png').read_bytes()).hexdigest(),'size':list(SIZE),'port_width':64,'runtime_validated':False,'rooms':[]};yy,xx=np.mgrid[:512,:512]
