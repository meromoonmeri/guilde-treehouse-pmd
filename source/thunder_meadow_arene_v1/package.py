...
.venv/bin/python source/thunder_meadow_arene_v1/package.py   (après build.py)
"""
...
REVIEW = ['scene_t000.png', 'scene_animee.webp', 'collisions_marqueurs.png', 'scene_eclair.png']
...
def main():
    subprocess.run([sys.executable, '-m', 'unittest', 'source.thunder_meadow_arene_v1.test_build'], cwd=R, check=True)
    M = json.loads((O / 'manifest.json').read_text())
    zipdir(O / f'{PFX}_projet_pmdo_0812.zip',
           [(p, 'thunder_meadow_arene/' + p.relative_to(S).as_posix()) for p in sorted(S.rglob('*')) if p.is_file()])
    ...
    items += [(O / f'{PFX}_thunder_meadow_arene_calques.ora', f'{PFX}_thunder_meadow_arene_calques.ora'),
    ...
    data = {'size': M['size_px'], 'loop': M['scene_loop_ticks'], 'stack': stack, 'surface': 'rgb(90,150,100)', 'poses': poses,
            ...
            'entry': M['access']['entry_px'], 'threshold': M['access']['arene_centre_px']}
    page = (HERE / 'viewer_template.html').read_text().replace('__DATA__', json.dumps(data))
    (R / 'apercu_thunder_meadow_arene_v1.html').write_text(page)
    for p in [O / f'{PFX}_projet_pmdo_0812.zip', O / f'{PFX}_calques_png_8px.zip', R / 'apercu_thunder_meadow_arene_v1.html']:
