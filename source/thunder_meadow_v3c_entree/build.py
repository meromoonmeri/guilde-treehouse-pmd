"""Thunder Meadow V3c — entrée sans rochers, chemin droit : montagne nord + grotte intégrée, nuages et foudre en contrebas.
...
REF = ROOT / "5394.png"
GUIDE = HERE / "generation" / "entree_guide_chemin_droit.png"
OUT = ROOT / "renders" / "thunder_meadow_v3c_entree"
PFX = "THUNDERMEADOW_V3C_"
W, H = 456, 336
...
manifest = {"reference": {"file": REF.name, "sha256": hashlib.sha256(REF.read_bytes()).hexdigest()},
            "guide_genere": {"file": "source/thunder_meadow_v3c_entree/generation/entree_guide_chemin_droit.png", "sha256": hashlib.sha256(GUIDE.read_bytes()).hexdigest(),
...
with zipfile.ZipFile(OUT.parent / "thunder_meadow_v3c_entree_pack.zip", "w", zipfile.ZIP_DEFLATED) as z:
    for p in sorted(OUT.rglob("*")):
        if p.is_file(): z.write(p, p.relative_to(OUT.parent))
    z.write(__file__, "thunder_meadow_v3c_entree/source/build.py"); z.write(GUIDE, "thunder_meadow_v3c_entree/source/entree_guide_chemin_droit.png")
