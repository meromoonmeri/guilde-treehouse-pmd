...
import json,io,struct,hashlib
R=Path(__file__).resolve().parents[1];O=R/'sprites/falaises_metano';M=json.loads((O/'kit.json').read_text());TS=json.loads((O/'Extension_Metano.tsj').read_text())
assert M['size_px']==[2048,1536] and M['grid_px']==8 and len(M['maps'])==3
atlas=Image.open(O/'Extension_Metano.png').convert('RGBA');cols=TS['columns'];count=TS['tilecount'];assert cols*8==atlas.width and count==atlas.width*atlas.height//64
...
b=(O/'Extension_Metano.tile').read_bytes();size,n=struct.unpack_from('<II',b);assert (size,n)==(8,count)
...
for desc in M['maps']:
    ...
    assert (d/tm['tilesets'][0]['source']).resolve()==(O/'Extension_Metano.tsj').resolve()
