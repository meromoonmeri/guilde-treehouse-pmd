...
OUT = ROOT / "renders" / "zone_d06p11_v10"; PFX = "D06P11_V10_"
...
terr = LD(G / "terrain_canonique.png"); assert terr.shape[:2] == (H, W)
prov = json.load(open(G / "provenance_tuiles.json")); col = np.array(prov["collisions_8px"]); walk = col == 0
...
with zipfile.ZipFile(OUT.parent / "zone_d06p11_v10_pack.zip", "w", zipfile.ZIP_DEFLATED) as z:
    for p in sorted(OUT.rglob("*")):
        if p.is_file(): z.write(p, p.relative_to(OUT.parent))
    z.write(__file__, "zone_d06p11_v10/source/build.py")
