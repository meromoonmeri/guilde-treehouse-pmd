"""Zone D06P11 V11 — calque FALAISE seul (roche ROM d06p11a sous tout le massif, faces cachées comprises).
Layout identique V10 ; seul changement : second chemin (rangées 41-50) élargi, sans lèvres rocheuses.
...
# V11 : second chemin vers la grotte ouvert (ancien goulet 5 cases entre deux lèvres) -> rampe de 13 cases, sans bordure
cls[41:51, 28:41] = "sable"
cls[40:42, 20:44][cls[40:42, 20:44] == "roche"] = "sable"   # raccord haut, supprime l'encoche
cls[49:51, 20:48][cls[49:51, 20:48] == "roche"] = "sable"   # raccord bas
...
for seg in segs:
    ...
    low = list(range(SB[rc] + 2, RH))   # V11 : saute la rangée de sable résiduel; sandr = list(range(ST[rc], SB[rc] + 1))
    if c == "sable":   # V11 : sous le sol -> mur ROM (face cachée), le sol est un calque séparé
        for n in range(j, k): put(n, i, 3 + ((n - KB + 3) % 15), rc)
...
Image.fromarray(terr.astype(np.uint8), "RGBA").save(G / "calque_falaise.png")
walk = (cls == "sable") | (cls == "garde")
lab_, _ = nd.label(walk); sz_ = np.bincount(lab_.ravel()); sz_[0] = 0; walk = lab_ == sz_.argmax()
meta = {"source_roche": "ROM d06p11a", "cls": [[str(v) for v in r] for r in cls], "collisions_8px": (~walk).astype(int).tolist(),
        "cases_marchables": int(walk.sum()), "bloc_grotte_px": [int(by0), int(bx0), int(bh), int(bw_)], "sol_grotte_rangee": int(oy + (st_c - CAVE_R0)),
        "tuiles": {f"{j},{i}": [int(pick[(j, i)][0]), int(pick[(j, i)][1])] for (j, i) in pick}}
(G / "falaise_meta.json").write_text(json.dumps(meta)); print(meta["cases_marchables"], meta["bloc_grotte_px"], meta["sol_grotte_rangee"])
