...
m = np.kron(cells, np.ones((T, T))).astype(float)
m = nd.gaussian_filter(m, 5.5) + nd.gaussian_filter(rng.standard_normal((H, W)), 1.2) * .10
ground = (m > .5) & island
ground[srow * T - 2:, bx0 + 16:bx0 + bw - 16] |= np.kron(cells, np.ones((T, T))).astype(bool)[srow * T - 2:, bx0 + 16:bx0 + bw - 16]
ground = nd.binary_opening(ground, np.ones((3, 3)))
Y, X = np.mgrid[:H, :W]
cx = bx0 + bw // 2
