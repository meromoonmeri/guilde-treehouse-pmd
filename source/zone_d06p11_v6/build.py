"""Zone D06P11 V6 — carte agrandie 544x640 : chemin et terrasses plus vastes, mer visible des deux côtés.
...
V5 = ROOT / "renders" / "zone_d06p11_v5"; V2 = ROOT / "renders" / "zone_d06p11_v2"
OUT = ROOT / "renders" / "zone_d06p11_v6"; PFX = "D06P11_V6_"
m5 = json.load(open(V5 / "manifest.json")); W0, H0 = m5["canvas"]; HOR = m5["horizon_y"]
...
# ---------- terrain ----------
g = np.array(Image.open(G / "falaise_etendue.png").convert("RGB").resize((W, H), Image.BOX)).astype(int)
...
# ---------- soleil animé généré ----------
sg = np.array(Image.open(G / "soleil_frames.png").convert("RGB")).astype(int)
...
def place(side_left, rad):
    ...
    for cx in xs[::4]:
        for cy in range(rad + 6, HOR - rad - 20, 4):
            if free[cy - rad - 4:cy + rad + 5, cx - rad - 4:cx + rad + 5].all():
                sc = -abs(cy - 40) - (abs(cx - (60 if side_left else W - 60)) * .3)
                if best is None or sc > best[0]: best = (sc, cx, cy)
    return best[1:] if best else None
SUNPOS = place(True, SUN_S // 2); MOONPOS = place(False, 18)
...
checks = {"cases_marchables": int(walk.sum()), "soleil_frames": NSUN, "soleil_centre": SUNPOS, "lune_centre": MOONPOS}
for mode, d5 in m5["modes"].items():
    D = OUT / mode
    sky5 = LD(V5 / d5["calques"]["00_ciel"]); rows = sky5[:, 0]
    ...
    for f in d5["etoiles"]["frames"]:
        e = LD(V5 / f); o = np.zeros((H, W, 4), int); o[:H0, :W0] = e; o[:H0, W0:] = e[:, 100:100 + W - W0]; ST.append(o)
    AS = []
    if mode == "nuit":
        mcx, mcy = m5["modes"]["nuit"]["astre"]["centre"]; R = 20
        for f in d5["astre"]["frames"]:
            a5 = LD(V5 / f); o = np.zeros((H, W, 4), int); cx, cy = MOONPOS
            o[cy - R:cy + R, cx - R:cx + R] = a5[mcy - R:mcy + R, mcx - R:mcx + R]; AS.append(o)
        FLA = d5["astre"]["FrameLength_60fps"]
    else:
        for (r_, m_) in SUN:
    ...
    nu = LD(V5 / d5["nuages"]["bande"]); ny = d5["nuages"]["y"]; bw = nu.shape[1]
    mer = [widen_sea(LD(V5 / f)) for f in d5["mer_bpa"]["frames"]]; pal = [widen_sea(LD(V5 / f)) for f in d5["mer_palette"]["frames"]]
    ter = grade(terr, mode)
    ...
    def comp(t):
        ...
        for a in (sky, ST[(fr // 8) % len(ST)], AS[(fr // FLA) % len(AS)], cl, mer[(fr // 10) % 10], pal[(fr // 5) % len(pal)], ter):
    ...
    man["modes"][mode] = {"ordre_calques": ["00_ciel", "01_etoiles", "02_astre", "03_nuages", "04_mer_bpa", "04b_mer_palette", "05_terrain"],
        ...
        "astre": {"frames": [f"{mode}/astre/{PFX}{mode}_02_astre_f{i:02d}.png" for i in range(len(AS))], "FrameLength_60fps": FLA, "ms": round(FLA * 1000 / 60, 1),
                  "centre": list(SUNPOS if mode != "nuit" else MOONPOS)},
...
with zipfile.ZipFile(OUT.parent / "zone_d06p11_v6_pack.zip", "w", zipfile.ZIP_DEFLATED) as z:
    for p in sorted(OUT.rglob("*")):
        if p.is_file(): z.write(p, p.relative_to(OUT.parent))
    z.write(__file__, "zone_d06p11_v6/source/build.py")
