...
OUT = ROOT / "renders" / "zone_d06p11_v7"; PFX = "D06P11_V7_"
...
# ---------- terrain ----------
g = np.array(Image.open(G / "falaise_sans_couronne.png").convert("RGB").resize((W, H), Image.BOX)).astype(int)
...
checks = {"cases_marchables": int(walk.sum()), "soleil_frames": NSUN, "lune_frames": NMOON, "soleil_centre": SUNPOS, "lune_centre": MOONPOS}
...
with zipfile.ZipFile(OUT.parent / "zone_d06p11_v7_pack.zip", "w", zipfile.ZIP_DEFLATED) as z:
    for p in sorted(OUT.rglob("*")):
        if p.is_file(): z.write(p, p.relative_to(OUT.parent))
    z.write(__file__, "zone_d06p11_v7/source/build.py")
